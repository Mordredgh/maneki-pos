...
let _roiPedidoData = null;
function saveEquipos() {
  sbSave("equipos", equipos);
}
function saveRoiHistorial() {
  ...
  if (editId) {
    ...
    if (idx !== -1) {
      equipos[idx] = {
        ...
        metaReemplazo,
        metaMensual,
        historialPagos: equipos[idx].historialPagos || []
...
function renderEquiposGrid() {
  ...
  grid.innerHTML = equipos.map((eq) => {
    ...
    const recuperadoMes = (eq.historialPagos || []).filter((h) => h.fecha && h.fecha.startsWith(mesActual)).reduce((s, h) => s + Number(h.monto || 0), 0);
    ...
    h.equiposIds.forEach((eqId) => {
      ...
      if (idx !== -1) {
        ...
        if (Array.isArray(equipos[idx].historialPagos)) {
          equipos[idx].historialPagos = equipos[idx].historialPagos.filter(
            (p) => p.pedidoId !== pedidoId && (!pedidoFolio || p.folio !== pedidoFolio)
          );
        }
      }
    });
...
function actualizarCalculoRoi() {
  ...
  const pedidoId = document.getElementById("roiPedidoId").value;
...
function cerrarRoiManualModal() {
  closeModal("roiManualModal");
}
...
function confirmarRoiManual() {
  ...
  saveEquipos();
  roiHistorial.push({
    fecha: window._fechaHoy ? window._fechaHoy() : (() => {
      const d = /* @__PURE__ */ new Date();
      return d.getFullYear() + "-" + ("0" + (d.getMonth() + 1)).slice(-2) + "-" + ("0" + d.getDate()).slice(-2);
    })(),
    folio: "__manual__",
