...
;
"use strict";function _levenshtein(t,o){const a=t.length,s=o.length,r=Array.from({length:a+1},(i,n)=>Array.from({length:s+1},(l,d)=>d===0?n:0));for(let i=1;i<=s;i++)r[0][i]=i;for(let i=1;i<=a;i++)for(let n=1;n<=s;n++)r[i][n]=t[i-1]===o[n-1]?r[i-1][n-1]:1+Math.min(r[i-1][n],r[i][n-1],r[i-1][n-1]);return r[a][s]}window._levenshtein=_levenshtein;function _fuzzyMatch(t,o,a=2){return t=t.toLowerCase().trim(),o=o.toLowerCase(),!t||o.includes(t)?!0:o.split(/[\s,.-]+/).some(r=>{const i=r.substring(0,t.length+2);return i.length>=t.length-1&&_levenshtein(t,i)<=a})}window._fuzzyMatch=_fuzzyMatch;function calcularProducibles(t){if(!Array.isArray(t.mpComponentes)||t.mpComponentes.length===0)return null;let o=1/0;for(const a of t.mpComponentes){const s=(window.products||[]).find(n=>String(n.id)===String(a.id));if(!s)return 0;const r=typeof getStockEfectivo=="function"?getStockEfectivo(s):s.stock||0,i=parseFloat(a.qty)||1;o=Math.min(o,Math.floor(r/i))}return o===1/0?0:o}window.calcularProducibles=calcularProducibles;function abrirBulkPrecioModal(){let t=document.getElementById("bulkPrecioModal");t||(t=document.createElement("div"),t.id="bulkPrecioModal",t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;",t.addEventListener("click",s=>{s.target===t&&(t.style.display="none")}),document.body.appendChild(t));const a=[...new Set((window.products||[]).map(s=>s.category).filter(Boolean))].map(s=>{const r=(window.categories||[]).find(i=>String(i.id)===String(s));return`<option value="${_esc(s)}">${_esc(r?r.emoji?r.emoji+" "+r.name:r.name:s)}</option>`}).join("");t.innerHTML=`
    <div style="background:#fff;border-radius:20px;width:min(540px,95vw);max-height:88vh;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,0.25);">
        ...
        <div style="padding:16px 24px;border-top:1px solid #f3f4f6;display:flex;gap:8px;justify-content:flex-end;">
    ...
    </div>`,t.style.display="flex",bulkPrecioPreview()}window.abrirBulkPrecioModal=abrirBulkPrecioModal;function _bulkPrecioGetAfectados(){const t=parseFloat(document.getElementById("bulkPrecioNum")?.value)||0,o=document.getElementById("bulkPrecioSoloPT")?.checked||!1,a=document.getElementById("bulkPrecioSoloMP")?.checked||!1,s=(document.getElementById("bulkPrecioCat")?.value||"").trim();return(window.products||[]).filter(r=>s&&String(r.category)!==s?!1:o&&a?!0:!(o&&!(!r.tipo||r.tipo==="producto"||r.tipo==="producto_interno"||r.tipo==="pack")||a&&r.tipo!=="materia_prima")).map(r=>{const i=a&&!o?"cost":"price",n=parseFloat(r[i])||0,l=Math.max(0,Math.round(n*(1+t/100)*100)/100);return{p:r,campoKey:i,precioActual:n,precioNuevo:l}}).filter(r=>r.precioActual>0)}function bulkPrecioPreview(){const t=document.getElementById("bulkPrecioPreviewList");if(!t)return;const o=_bulkPrecioGetAfectados();if(!o.length){t.innerHTML='<p style="font-size:.78rem;color:#9ca3af;text-align:center;padding:16px;">Sin productos que coincidan con los filtros</p>';return}t.innerHTML=o.slice(0,50).map(({p:a,campoKey:s,precioActual:r,precioNuevo:i})=>{const n=i-r,l=n>0?"#16a34a":n<0?"#dc2626":"#6b7280",d=s==="cost"?"Costo":"Precio";return`<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 8px;border-bottom:1px solid #f3f4f6;font-size:.78rem;">
            ...
            <span style="color:#6b7280;white-space:nowrap;margin:0 8px;">${d}: $${r.toFixed(2)}</span>
            <span style="font-weight:700;color:${l};white-space:nowrap;">\u2192 $${i.toFixed(2)}</span>
        </div>`}).join("")+(o.length>50?`<p style="font-size:.72rem;color:#9ca3af;text-align:center;padding:8px;">...y ${o.length-50} m\xE1s</p>`:"")}window.bulkPrecioPreview=bulkPrecioPreview;async function bulkPrecioAplicar(){const t=_bulkPrecioGetAfectados();if(!t.length){manekiToastExport("Sin productos que actualizar","warn");return}bulkPrecioPreview();const o=parseFloat(document.getElementById("bulkPrecioNum")?.value)||0,a=document.getElementById("bulkPrecioSoloMP")?.checked&&!document.getElementById("bulkPrecioSoloPT")?.checked?"costo":"precio",s=o>0?"+":"",r=t.slice(0,5).map(({p:i,precioActual:n,precioNuevo:l})=>`<div style="display:flex;justify-content:space-between;font-size:.8rem;padding:3px 0;border-bottom:1px solid #f3f4f6;">
        ...
        </div>`;return}const h=(document.getElementById("inventorySearch")||{}).value?.trim().toLowerCase()||"",w=(document.getElementById("inventoryTagFilter")||{}).value||"",B=(document.getElementById("inventoryProveedorFilter")||{}).value?.trim().toLowerCase()||"";function b(e){const m=window._normSearch||(y=>String(y||"").toLowerCase()),f=m(h),k=m(B),$=y=>!w||y.tags&&y.tags.includes(w),E=y=>!B||m(y.proveedor||"").includes(k);if(!h)return e.filter(y=>$(y)&&E(y));const S=e.filter(y=>(m(y.name).includes(f)||m(y.sku||"").includes(f)||m(y.proveedor||"").includes(f)||m(y.notas||"").includes(f)||(y.tags||[]).some(W=>m(W).includes(f)))&&$(y)&&E(y));return S.length>0?S:e.filter(y=>(_fuzzyMatch(f,y.name||"")||_fuzzyMatch(f,y.sku||"")||_fuzzyMatch(f,y.proveedor||""))&&$(y)&&E(y))}const L=b(n.filter(e=>e.tipo==="materia_prima")),O=b(n.filter(e=>e.tipo==="servicio")),G=b(n.filter(e=>e.tipo==="producto_variable")),M=b(n.filter(e=>!e.tipo||e.tipo==="producto"||e.tipo==="producto_interno"||e.tipo==="pack"));function C(e){if(!window._invSortCol)return e;const m=window._invSortCol,f=window._invSortDir;return[...e].sort((k,$)=>{let E,S;return m==="name"?(E=(k.name||"").toLowerCase(),S=($.name||"").toLowerCase()):m==="sku"?(E=(k.sku||"").toLowerCase(),S=($.sku||"").toLowerCase()):m==="category"?(E=(k.category||"").toLowerCase(),S=($.category||"").toLowerCase()):m==="price"?(E=Number(k.price)||0,S=Number($.price)||0):m==="stock"?(E=Number(k.stock)||0,S=Number($.stock)||0):m==="margin"&&(E=k.cost&&k.price?(k.price-k.cost)/k.price:-1,S=$.cost&&$.price?($.price-$.cost)/$.price:-1),E<S?f==="asc"?-1:1:E>S?f==="asc"?1:-1:0})}function I(e,m){const f=String(e.id),k=l.get(f)??(typeof getStockEfectivo=="function"?getStockEfectivo(e):parseInt(e.stock)||0),$=e.imageUrl?`<img src="${e.imageUrl}" alt="${_esc(e.name||"")}" style="width:40px;height:40px;object-fit:cover;border-radius:8px;" loading="lazy">`:`<span style="font-size:1.6rem;">${e.image||"\u{1F3ED}"}</span>`;let E;k===0?E='<span class="badge-danger">Agotado</span>':k<=(e.stockMin||5)?E='<span class="badge-warning">Bajo Stock</span>':E='<span class="badge-success">Disponible</span>';const S=(window.categories||[]).find(F=>F.id===e.category),y=S?S.name:e.category||"";return`
        ...
        <tr style="animation:mkSectionIn 0.3s ease both;animation-delay:${m*.03}s" class="hover:bg-amber-50">
            ...
            <td class="px-4 py-3">
                <div>
                    ...
                    ${(()=>{const x=calcularProducibles(e);if(x===null)return"";const T=x>=5?"#16a34a":x>=1?"#d97706":"#dc2626",A=x>=5?"#d1fae5":x>=1?"#fef3c7":"#fee2e2",H=x===0?"Sin stock MP":`Producibles: ${x}`;return`<div style="margin-top:3px;"><span style="font-size:9px;font-weight:700;padding:1px 7px;border-radius:99px;background:${A};color:${T};border:1px solid ${T}33;">\u{1F3ED} ${H}</span></div>`})()}
                </div>
            </td>
            <td class="px-4 py-3 text-gray-500 text-xs inv-col-hidden-sku">${_esc(e.sku||"\u2014")}</td>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px;">
        ...
        </div>`)}function invSectionPage(t,o){const a=`_invPage_${t}`,s=window.products||[],r=t==="mp"?s.filter(c=>c.tipo==="materia_prima"):t==="svc"?s.filter(c=>c.tipo==="servicio"):t==="pv"?s.filter(c=>c.tipo==="producto_variable"):s.filter(c=>!c.tipo||c.tipo==="producto"||c.tipo==="producto_interno"||c.tipo==="pack"),i=(document.getElementById("inventorySearch")||{}).value?.trim().toLowerCase()||"",n=(document.getElementById("inventoryTagFilter")||{}).value||"",l=(document.getElementById("inventoryProveedorFilter")||{}).value?.trim().toLowerCase()||"",d=r.filter(c=>{const h=!i||c.name.toLowerCase().includes(i)||(c.sku||"").toLowerCase().includes(i)||(c.proveedor||"").toLowerCase().includes(i)||(c.notas||"").toLowerCase().includes(i)||(c.tags||[]).some(b=>b.toLowerCase().includes(i)),w=!n||c.tags&&c.tags.includes(n),B=!l||(c.proveedor||"").toLowerCase().includes(l);return h&&w&&B}),g=Math.max(1,Math.ceil(d.length/(window._invPageSize||10)));window[a]=Math.max(1,Math.min(o,g)),renderInventoryTable()}window.invSectionPage=invSectionPage;function _renderInventoryPagination(t,o,a,s,r){let i=document.getElementById("inventoryPaginationBar");if(!i){const g=document.getElementById("inventoryTable")?.closest('table, .overflow-x-auto, [class*="overflow"]');if(!g)return;i=document.createElement("div"),i.id="inventoryPaginationBar",g.insertAdjacentElement("afterend",i)}if(o<=1&&a<=r){i.innerHTML="";return}const n=Math.min(s+r,a),l=`Mostrando <b>${s+1}\u2013${n}</b> de <b>${a}</b> productos`;function d(){const g=[],c=(h,w)=>{for(let B=h;B<=w;B++)g.push(B)};return o<=7?c(1,o):(g.push(1),t>4&&g.push("..."),c(Math.max(2,t-2),Math.min(o-1,t+2)),t<o-3&&g.push("..."),g.push(o)),g.map(h=>{if(h==="...")return'<span style="padding:0 4px;color:#9ca3af;">\u2026</span>';const w=h===t;return`<button onclick="invGoToPage(${h})"
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;
            ...
            <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                ...
                <select onchange="invChangePageSize(this.value)"
                    ...
                    ${[10,25,50,100].map(g=>`<option value="${g}" ${g===r?"selected":""}>${g} por p\xE1gina</option>`).join("")}
                </select>
            </div>
...
`});const G=new Blob([O],{type:"text/csv;charset=utf-8;"}),M=URL.createObjectURL(G),C=document.createElement("a");C.href=M,C.download=`movimientos-${i}.csv`,C.click(),URL.revokeObjectURL(M)},o.parentNode.insertBefore(w,o)),!r.length){o.innerHTML='<p class="text-gray-400 text-sm text-center py-4">Sin movimientos registrados</p>';return}const B={entrada:"\u{1F7E2}",salida:"\u{1F534}",ajuste:"\u{1F7E1}",creacion:"\u{1F535}",venta:"\u{1F7E0}",merma:"\u{1F7E4}"};o.innerHTML=r.slice(0,200).map(b=>{const L=new Date(b.fecha).toLocaleString("es-MX",{dateStyle:"short",timeStyle:"short"}),O=b.cantidad>=0?`+${b.cantidad}`:`${b.cantidad}`;return`<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:13px;">
            ...
            <div style="flex:1;min-width:0;">
                ...
                <div style="color:#6b7280;font-size:11px;">${r} \xB7 ${s.tipo} \xB7 ${_esc(s.motivo||"Sin motivo")}</div>
            </div>
            <div style="text-align:right;white-space:nowrap;">
        ...
        </div>`}).join("")}window.renderStockMovements=renderStockMovements;function duplicarProducto(t){const o=(window.products||[]).find(s=>String(s.id)===String(t));if(!o){manekiToastExport("Producto no encontrado","err");return}const a=JSON.parse(JSON.stringify(o));a.id=_genId(),a.name="Copia de "+o.name,a.sku=(o.sku||"")+"-C",a.stock=0,a.historialPrecios=[],a.historialCostos=[],window.products.unshift(a),saveProducts(),renderInventoryTable(),manekiToastExport(`\u{1F4CB} "${a.name}" creado \u2014 ed\xEDtalo para ajustar stock y SKU`,"ok")}window.duplicarProducto=duplicarProducto;function abrirReporteRentabilidad(){const t=(window.products||[]).filter(n=>!n.tipo||n.tipo==="producto"||n.tipo==="producto_interno"),o=t.map(n=>{const l=n.price>0&&n.cost>0?(n.price-n.cost)/n.price*100:null;return{...n,_margen:l}}).sort((n,l)=>(l._margen??-1/0)-(n._margen??-1/0)),a=o.map((n,l)=>{const d=n._margen!==null?n._margen.toFixed(1)+"%":"\u2014",g=n.price>0&&n.cost>0?"$"+(n.price-n.cost).toFixed(2):"\u2014",c=n._margen===null?"#9ca3af":n._margen>=50?"#16a34a":n._margen>=30?"#d97706":"#dc2626";return`<tr style="border-bottom:1px solid #f3f4f6;">
...
`),i=new Blob([r],{type:"text/csv;charset=utf-8;"}),n=URL.createObjectURL(i),l=document.createElement("a");l.href=n,l.download="inventario_seleccion.csv",l.click(),URL.revokeObjectURL(n),manekiToastExport(`\u{1F4E5} ${o.length} productos exportados`,"ok")}window.invBulkExportar=invBulkExportar;async function invBulkCambiarCategoria(){const t=invGetSelectedIds();if(!t.length)return;const o=await new Promise(s=>{const r=document.getElementById("mkBatchCatModal");r&&r.remove();const n=(window.categories||[]).map(d=>`<option value="${d.id}">${d.emoji||""} ${d.name}</option>`).join(""),l=document.createElement("div");l.id="mkBatchCatModal",l.className="mk-modal-overlay",l.innerHTML=`<div class="mk-modal-box" style="max-width:360px">
    ...
    `;const s=document.getElementById("mkInvFilterInfo");s?s.parentElement.insertBefore(a,s):o.parentElement.insertBefore(a,o.nextSibling)}}}function _mkInvCounterChips(){const t=document.getElementById("mkInvFilterInfo");if(!t)return;const o=document.getElementById("invDualContainer"),a=o?o.querySelectorAll(".inv-bulk-cb").length:0,s=(window.products||[]).length,r=document.getElementById("inventorySearch"),i=document.getElementById("inventoryTagFilter"),n=document.getElementById("inventoryProveedorFilter"),l=document.getElementById("inventoryTipoFilter"),d=[];r&&r.value.trim()&&d.push(`<span class="mk-filter-chip">Buscar: ${_esc(r.value.trim())}<button data-tip="Quitar" onclick="_mkInvClearOne('inventorySearch')">\u2715</button></span>`),l&&l.value&&d.push(`<span class="mk-filter-chip">Tipo: ${_esc(_MK_TIPO_LABELS[l.value]||l.value)}<button data-tip="Quitar" onclick="_mkInvSetTipo('')">\u2715</button></span>`),i&&i.value&&d.push(`<span class="mk-filter-chip">Tag: ${_esc(i.value)}<button data-tip="Quitar" onclick="_mkInvClearOne('inventoryTagFilter')">\u2715</button></span>`),n&&n.value&&d.push(`<span class="mk-filter-chip">Proveedor: ${_esc(n.options[n.selectedIndex]?.text||n.value)}<button data-tip="Quitar" onclick="_mkInvClearOne('inventoryProveedorFilter')">\u2715</button></span>`);let g=`<span class="mk-result-count">Mostrando <b>${a}</b> de ${s} producto${s!==1?"s":""}</span>`;d.length&&(g+=`<div class="mk-filter-chips">${d.join("")}<button class="mk-filter-clear" onclick="_mkInvClearFilters()">Limpiar todo</button></div>`),t.innerHTML=g,_mkInvSyncSeg()}function _mkInvSummaryRow(){const t=document.getElementById("invDualContainer");if(!t||!t.parentElement)return;const o=new Set([...t.querySelectorAll(".inv-bulk-cb")].map(l=>String(l.dataset.id))),a=window._invStockCache;let s=0,r=0,i=0;(window.products||[]).forEach(l=>{if(!o.has(String(l.id)))return;i++;const d=a?.get(String(l.id))??(Number(l.stock)||0);s+=(Number(l.cost)||0)*Math.max(0,d),d<=(Number(l.stockMin)||5)&&r++});let n=document.getElementById("mkInvSummary");if(i===0){n&&n.remove();return}n||(n=document.createElement("div"),n.id="mkInvSummary",n.className="mk-table-summary",n.style.cssText="display:flex;gap:18px;align-items:center;flex-wrap:wrap;padding:10px 18px;border-radius:0 0 14px 14px;margin-top:-2px;",t.parentElement.insertBefore(n,t.nextSibling)),n.innerHTML=`<span>Valor en costo: <b>$${s.toLocaleString("es-MX",{maximumFractionDigits:0})}</b></span><span style="color:var(--tx-muted);">${i} producto${i!==1?"s":""}</span>`+(r>0?`<span style="color:#dc2626;font-weight:800;">\u26A0 ${r} bajo stock</span>`:'<span style="color:#059669;font-weight:700;">\u2713 stock saludable</span>')}(function(){const o=window.renderInventoryTable;if(typeof o!="function"||o._mkWrapped)return;const a=function(...s){const r=o.apply(this,s);try{_mkInvToolbarOnce(),_mkInvCounterChips(),_mkInvSummaryRow()}catch{}return r};a._mkWrapped=!0,window.renderInventoryTable=a})();function _mkInvModal(t,o,a,s="700px"){let r=document.getElementById(t+"_ov");r||(r=document.createElement("div"),r.id=t+"_ov",r.style.cssText="position:fixed;inset:0;z-index:9100;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;padding:16px;",document.body.appendChild(r)),r.innerHTML=`
    ...
    </div>`,r.onclick=i=>{i.target===r&&r.remove()},r.style.display="flex"}function abrirConteoFisico(){const t=(window.products||[]).filter(r=>r.tipo!=="servicio"&&r.activo!==!1);if(!t.length){typeof manekiToastExport=="function"&&manekiToastExport("Sin productos para contar","warn");return}const o=typeof window._esc=="function"?window._esc:r=>String(r||""),s=`
    ...
    <table style="width:100%;border-collapse:collapse;">
      ...
      <tbody>${t.map((r,i)=>{const n=typeof getStockEfectivo=="function"?getStockEfectivo(r):Number(r.stock)||0;return`<tr style="${i%2?"background:#f9fafb":""}">
      <td style="padding:7px 10px;font-weight:600;font-size:.85rem;">${o(r.name)}</td>
      <td style="padding:7px 10px;text-align:center;color:#6b7280;font-size:.82rem;">${o(r.category||"\u2014")}</td>
      <td style="padding:7px 10px;text-align:center;font-weight:700;">${n}</td>
      <td style="padding:7px 10px;text-align:center;">
        <input type="number" min="0" value="${n}" data-pid="${o(r.id)}" data-sistema="${n}"
    ...
    <div style="margin-top:18px;display:flex;gap:10px;justify-content:flex-end;">
      ...
      <button onclick="_mkAplicarConteoFisico()" style="padding:9px 24px;border-radius:10px;background:linear-gradient(135deg,#C5973B,#a8864f);color:white;border:none;cursor:pointer;font-weight:700;">\u2705 Aplicar ajustes</button>
    </div>`;_mkInvModal("mkConteo","\u{1F4CB} Conteo F\xEDsico de Inventario",s,"780px")}window.abrirConteoFisico=abrirConteoFisico,window._mkAplicarConteoFisico=function(){const t=document.querySelectorAll("#mkConteo_ov .conteo-input");let o=0;if(t.forEach(a=>{const s=a.dataset.pid,r=Number(a.dataset.sistema),i=Number(a.value);if(isNaN(i)||i===r)return;const n=(window.products||[]).find(d=>String(d.id)===String(s));if(!n)return;const l=i-r;n.stock=i,typeof registrarMovimiento=="function"&&registrarMovimiento({productoId:n.id,productoNombre:n.name,tipo:l>0?"entrada_manual":"salida_manual",cantidad:Math.abs(l),motivo:"Conteo f\xEDsico",stockAntes:r,stockDespues:i}),o++}),o===0){typeof manekiToastExport=="function"&&manekiToastExport("Sin diferencias que ajustar","warn");return}typeof saveProducts=="function"&&saveProducts(),typeof renderInventoryTable=="function"&&renderInventoryTable(),document.getElementById("mkConteo_ov")?.remove(),typeof manekiToastExport=="function"&&manekiToastExport(`\u2705 ${o} ajuste${o!==1?"s":""} aplicados`,"ok")};function abrirReabastecimiento(){const t=(window.products||[]).filter(r=>r.tipo==="servicio"||r.activo===!1?!1:(typeof getStockEfectivo=="function"?getStockEfectivo(r):Number(r.stock)||0)<=(Number(r.stockMin)||5));if(!t.length){typeof manekiToastExport=="function"&&manekiToastExport("\u2705 Sin productos bajo stock m\xEDnimo","ok");return}const o=typeof window._esc=="function"?window._esc:r=>String(r||""),a={};t.forEach(r=>{const i=r.proveedor||"Sin proveedor";a[i]||(a[i]=[]),a[i].push(r)});const s=Object.entries(a).map(([r,i])=>{const n=o(r),l=i.map(c=>{const h=typeof getStockEfectivo=="function"?getStockEfectivo(c):Number(c.stock)||0,w=Number(c.stockMin)||5,B=Math.max(1,w*2-h);return`<tr><td style="padding:6px 10px;font-size:.83rem;font-weight:600;">${o(c.name)}</td>
        ...
        <td style="padding:6px 10px;text-align:center;font-size:.82rem;font-weight:700;color:#C5973B;">${B}</td>
...
`),s=document.createElement("a");s.href=URL.createObjectURL(new Blob([a],{type:"text/csv;charset=utf-8;"}));const r=new Date,i=`${r.getFullYear()}-${String(r.getMonth()+1).padStart(2,"0")}-${String(r.getDate()).padStart(2,"0")}`;s.download=`reabastecimiento_${i}.csv`,s.click()};function mostrarDonutCategoria(){const t=typeof window._esc=="function"?window._esc:l=>String(l||""),o={};(window.products||[]).forEach(l=>{if(l.tipo==="servicio"||l.activo===!1)return;const d=typeof getStockEfectivo=="function"?getStockEfectivo(l):Number(l.stock)||0,g=(Number(l.price)||0)*d,c=l.category||"Sin categor\xEDa";o[c]=(o[c]||0)+g});const a=Object.entries(o).sort((l,d)=>d[1]-l[1]),s=a.reduce((l,[,d])=>l+d,0),r=["#C5973B","#7c3aed","#10b981","#3b82f6","#f59e0b","#ef4444","#06b6d4","#8b5cf6","#f97316","#14b8a6"],i=a.map(([l,d],g)=>{const c=s>0?(d/s*100).toFixed(1):"0";return`<tr>
      <td style="padding:6px 12px;">
        <span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${r[g%r.length]};margin-right:6px;"></span>
        ${t(l)}
      </td>
    ...
    <table style="width:100%;border-collapse:collapse;">
      ...
      <tbody>${s.map(n=>{const l=a[String(n.id)]||0,d=l/60,g=Math.max(1,Math.ceil(d*14)),c=Number(n.stockMin)||0,h=g!==c?`<span style="color:${g>c?"#10b981":"#f59e0b"};font-weight:700;">${g>c?"\u25B2":"\u25BC"} ${g}</span>`:`<span style="color:#6b7280;">${g} (sin cambio)</span>`;return`<tr>
    ...
    <div style="margin-top:18px;display:flex;gap:10px;justify-content:flex-end;">
      ...
      <text x="${z+h}" y="${c-1}" text-anchor="middle" font-size="8" fill="#9ca3af">${u.label}</text>`}).join(""),B=i.length===0?'<p style="text-align:center;color:#9ca3af;padding:20px 0;font-size:.85rem;">Sin movimientos en los \xFAltimos 90 d\xEDas</p>':`
  ...
  `,I=`
    ...
    ${C}`;_mkInvModal("mkMovProd",`\u{1F4C8} Movimientos \u2014 ${o(a.name||"Producto")} (90d)`,I,"780px")}window.abrirMovimientoProducto=abrirMovimientoProducto;function abrirTendenciaInventario(){const t=window.inventarioSnapshots||[];if(t.length===0){typeof manekiToastExport=="function"&&manekiToastExport("Sin datos hist\xF3ricos a\xFAn. Los snapshots se generan autom\xE1ticamente.","warn");return}const o=[...t].sort((M,C)=>(M.fecha||"").localeCompare(C.fecha||"")),a=o.map(M=>M.fecha||""),s=o.map(M=>Number(M.valorTotal||M.valor||0)),r=540,i=140,n=Math.max(1,...s),l=Math.min(...s),d=n-l||1,c=`<polyline points="${s.map((M,C)=>{const I=20+C/Math.max(1,s.length-1)*(r-40),u=i-20-(M-l)/d*(i-40);return`${I},${u}`}).join(" ")}" fill="none" stroke="#6366f1" stroke-width="2.5" stroke-linejoin="round"/>`,h=s.map((M,C)=>{const I=20+C/Math.max(1,s.length-1)*(r-40),u=i-20-(M-l)/d*(i-40);return`<circle cx="${I}" cy="${u}" r="3.5" fill="#6366f1" opacity=".9"><title>${a[C]}: $${M.toLocaleString("es-MX")}</title></circle>`}).join(""),w=a.filter((M,C)=>C===0||C===a.length-1||C%Math.ceil(a.length/6)===0).map((M,C,I)=>`<text x="${20+a.indexOf(M)/Math.max(1,a.length-1)*(r-40)}" y="${i-2}" text-anchor="middle" font-size="9" fill="#9ca3af">${M.slice(5)}</text>`).join(""),B=s[s.length-1]||0,b=s[0]||0,L=b>0?((B-b)/b*100).toFixed(1):"\u2014",O=Number(L)>=0?"#10b981":"#ef4444",G=`
    <div style="display:flex;gap:12px;margin-bottom:14px;flex-wrap:wrap;">
      <div style="flex:1;min-width:100px;background:#eff6ff;border-radius:10px;padding:10px 14px;text-align:center;">
        <div style="font-size:1.2rem;font-weight:800;color:#4f46e5;">$${B.toLocaleString("es-MX",{maximumFractionDigits:0})}</div>
    ...
    <div style="background:#f9fafb;border-radius:10px;padding:12px;margin-bottom:14px;">
      <svg viewBox="0 0 ${r} ${i}" width="100%" height="140" style="display:block;overflow:visible;">
        <line x1="20" y1="${i-20}" x2="${r-10}" y2="${i-20}" stroke="#e5e7eb" stroke-width="1"/>
        ${c}${h}${w}
      </svg>
