...
`),s=document.createElement("a");s.href=URL.createObjectURL(new Blob([a],{type:"text/csv;charset=utf-8;"})),s.download=`reabastecimiento_${new Date().toISOString().split("T")[0]}.csv`,s.click()};function mostrarDonutCategoria(){const t=typeof window._esc=="function"?window._esc:l=>String(l||""),o={};(window.products||[]).forEach(l=>{if(l.tipo==="servicio"||l.activo===!1)return;const c=typeof getStockEfectivo=="function"?getStockEfectivo(l):Number(l.stock)||0,g=(Number(l.price)||0)*c,d=l.category||"Sin categor\xEDa";o[d]=(o[d]||0)+g});const a=Object.entries(o).sort((l,c)=>c[1]-l[1]),s=a.reduce((l,[,c])=>l+c,0),n=["#C5A572","#7c3aed","#10b981","#3b82f6","#f59e0b","#ef4444","#06b6d4","#8b5cf6","#f97316","#14b8a6"],i=a.map(([l,c],g)=>{const d=s>0?(c/s*100).toFixed(1):"0";return`<tr>
  ...
  `,B=`
    ...
    ${C}`;_mkInvModal("mkMovProd",`\u{1F4C8} Movimientos \u2014 ${o(a.name||"Producto")} (90d)`,B,"780px")}window.abrirMovimientoProducto=abrirMovimientoProducto;function abrirTendenciaInventario(){const t=window.inventarioSnapshots||[];if(t.length===0){typeof manekiToastExport=="function"&&manekiToastExport("Sin datos hist\xF3ricos a\xFAn. Los snapshots se generan autom\xE1ticamente.","warn");return}const o=[...t].sort((M,C)=>(M.fecha||"").localeCompare(C.fecha||"")),a=o.map(M=>M.fecha||""),s=o.map(M=>Number(M.valorTotal||M.valor||0)),n=540,i=140,r=Math.max(1,...s),l=Math.min(...s),c=r-l||1,d=`<polyline points="${s.map((M,C)=>{const B=20+C/Math.max(1,s.length-1)*(n-40),u=i-20-(M-l)/c*(i-40);return`${B},${u}`}).join(" ")}" fill="none" stroke="#6366f1" stroke-width="2.5" stroke-linejoin="round"/>`,h=s.map((M,C)=>{const B=20+C/Math.max(1,s.length-1)*(n-40),u=i-20-(M-l)/c*(i-40);return`<circle cx="${B}" cy="${u}" r="3.5" fill="#6366f1" opacity=".9"><title>${a[C]}: $${M.toLocaleString("es-MX")}</title></circle>`}).join(""),w=a.filter((M,C)=>C===0||C===a.length-1||C%Math.ceil(a.length/6)===0).map((M,C,B)=>`<text x="${20+a.indexOf(M)/Math.max(1,a.length-1)*(n-40)}" y="${i-2}" text-anchor="middle" font-size="9" fill="#9ca3af">${M.slice(5)}</text>`).join(""),I=s[s.length-1]||0,b=s[0]||0,L=b>0?((I-b)/b*100).toFixed(1):"\u2014",O=Number(L)>=0?"#10b981":"#ef4444",Y=`
    <div style="display:flex;gap:12px;margin-bottom:14px;flex-wrap:wrap;">
      <div style="flex:1;min-width:100px;background:#eff6ff;border-radius:10px;padding:10px 14px;text-align:center;">
