...
"use strict";function injectPtModal(){const e=document.getElementById("ptModal");e&&e.remove();const t=document.createElement("div");t.id="ptModal",t.className="modal",t.innerHTML=`
    <div style="background:#fff;border-radius:20px;box-shadow:0 32px 80px rgba(21,4,50,0.2);max-width:680px;width:100%;margin:auto;max-height:94vh;overflow-y:auto;padding:32px;">
        ...
        </div>`}).join(""),calcularDisponibilidadPt()}window.renderPtMpList=renderPtMpList;function recalcularCostoPt(){const e=window._ptMpComponentes||[],t=e.reduce((n,r)=>n+r.qty*r.costUnit,0),o=document.getElementById("ptCosto");o&&(o.value=t.toFixed(2));const i=document.getElementById("ptCostoDesglose");i&&e.length?i.textContent=e.map(n=>`${n.nombre} \xD7${n.qty} = $${(n.qty*n.costUnit).toFixed(2)}`).join(" \xB7 "):i&&(i.textContent=""),ptMostrarMargenInfo(),calcularDisponibilidadPt()}window.recalcularCostoPt=recalcularCostoPt;function calcularDisponibilidadPt(){const e=window._ptMpComponentes||[],t=document.getElementById("ptDisponibilidadBox"),o=document.getElementById("ptDisponibilidadNum"),i=document.getElementById("ptDisponibilidadDetalle");if(!t||!e.length){t&&(t.style.display="none");return}t.style.display="block";let n=1/0;const r=e.map(s=>{const d=(window.products||[]).find(x=>String(x.id)===String(s.id));if(d&&d.tipo==="servicio")return`${s.nombre}: \u2699\uFE0F servicio (sin l\xEDmite de stock)`;const l=d&&d.stock||0,m=s.qty>0?Math.floor(l/s.qty):0;return m<n&&(n=m),`${s.nombre}: ${l} uds \xF7 ${s.qty} = ${m} piezas`});isFinite(n)||(n=0),o&&(o.textContent=n,o.style.color=n>0?"#059669":"#ef4444"),i&&(i.innerHTML=r.join("<br>"))}window.calcularDisponibilidadPt=calcularDisponibilidadPt;function ptAplicarMargen(e){const t=parseFloat(document.getElementById("ptCosto")?.value||0)||0;if(!t){manekiToastExport("\u26A0\uFE0F Define el costo primero","warn");return}const o=t*(1+e/100),i=document.getElementById("ptPrecio");i&&(i.value=o.toFixed(2),ptMostrarMargenInfo())}window.ptAplicarMargen=ptAplicarMargen;function ptAplicarMargenCustom(){const e=parseFloat(document.getElementById("ptMargenCustom")?.value||0);!e||e<=0||ptAplicarMargen(e)}window.ptAplicarMargenCustom=ptAplicarMargenCustom;function ptActualizarPrecioSugerido(){ptMostrarMargenInfo()}window.ptActualizarPrecioSugerido=ptActualizarPrecioSugerido;function ptMostrarMargenInfo(){const e=parseFloat(document.getElementById("ptCosto")?.value||0)||0,t=parseFloat(document.getElementById("ptPrecio")?.value||0)||0,o=document.getElementById("ptMargenInfo");if(!o)return;if(!e||!t){o.textContent="";return}const i=((t-e)/t*100).toFixed(1),n=(t-e).toFixed(2),r=parseFloat(i)>=40?"#059669":parseFloat(i)>=20?"#d97706":"#ef4444";o.innerHTML=`<span style="color:${r};font-weight:700;">${i}% de margen</span> \xB7 Ganancia por pieza: <b style="color:${r};">$${n}</b>`}window.ptMostrarMargenInfo=ptMostrarMargenInfo;async function guardarProductoTerminado(){const e=p=>{const u=document.getElementById(p);return u?u.value:""},t=e("ptNombre").trim(),o=e("ptSku").trim(),i=e("ptCategory"),n=parseFloat(e("ptCosto"))||0,r=parseFloat(e("ptPrecio"))||0,s=parseInt(e("ptStockMin"))||5,d=parseFloat(e("ptRendimientoPorHoja"))||0,l=e("ptProveedorNombre").trim(),m=e("ptProveedorNotas").trim();if(!t){manekiToastExport("\u26A0\uFE0F El nombre es requerido","warn"),document.getElementById("ptNombre")?.focus();return}if(!r||r<=0){manekiToastExport("\u26A0\uFE0F El precio de venta debe ser mayor a $0","warn"),document.getElementById("ptPrecio")?.focus();return}if(r<n){manekiToastExport("\u26A0\uFE0F El precio no puede ser menor al costo","warn"),document.getElementById("ptPrecio")?.focus();return}const x=window.modoEdicion?window.edicionProductoId:null,h=(window.products||[]).find(p=>(p.name||"").trim().toLowerCase()===t.toLowerCase()&&String(p.id)!==String(x));if(h){manekiToastExport(`\u26A0\uFE0F Ya existe un producto llamado "${h.name}". Usa un nombre diferente o edita el existente.`,"warn"),document.getElementById("ptNombre")?.focus();return}if(typeof _fuzzyMatch=="function"){const p=(window.products||[]).find(u=>{if(String(u.id)===String(x))return!1;const c=t.toLowerCase(),f=(u.name||"").toLowerCase();if(c===f)return!1;const b=Math.max(c.length,f.length);return b<4?!1:typeof window._levenshtein=="function"?1-window._levenshtein(c,f)/b>=.8:!1});p&&manekiToastExport(`\u26A0\uFE0F Nombre similar a "${p.name}" ya existente. Si es diferente, contin\xFAa guardando.`,"warn")}if(o&&!skuEsUnico(o,x)){manekiToastExport(`\u26A0\uFE0F El SKU "${o}" ya est\xE1 en uso`,"warn");return}let g=n;const I=window._ptMpComponentes||[];if(g===0)if(I.length>0){const p=I.reduce((u,c)=>{const f=(window.products||[]).find(b=>String(b.id)===String(c.id));return u+(c.qty||0)*(f&&f.cost?f.cost:c.costUnit||0)},0);if(p>0&&await showConfirm(`El costo calculado basado en materias primas es $${p.toFixed(2)}. \xBFDeseas usarlo como costo del producto?`)){g=p;const c=document.getElementById("ptCosto");c&&(c.value=g.toFixed(2))}}else manekiToastExport("\u26A0\uFE0F El costo del producto est\xE1 en $0. Considera agregar un costo.","warn");const k=document.getElementById("ptSubmitBtn"),C=typeof btnLoading=="function"?btnLoading(k):()=>{};k&&(k.disabled=!0);try{if(window.currentProductImageFile){manekiToastExport("\u23F3 Subiendo imagen principal...","ok");const a=await subirImagenStorage(window.currentProductImageFile).catch(()=>null);a?window.currentProductImage=a:manekiToastExport("\u26A0\uFE0F No se pudo subir la imagen principal. Intenta de nuevo.","warn"),window.currentProductImageFile=null}const p=window._ptGaleriaFiles||[];if(p.length>0){manekiToastExport(`\u23F3 Subiendo ${p.length} foto(s) de galer\xEDa...`,"ok");const a=await Promise.all(p.map(v=>subirImagenStorage(v).catch(()=>null))),y=a.filter(Boolean),w=a.filter(v=>v===null).length;w>0&&manekiToastExport(`\u26A0\uFE0F ${w} foto(s) de galer\xEDa no se pudieron subir.`,"warn"),window._ptGaleriaUrls=[...window._ptGaleriaUrls||[],...y],window._ptGaleriaFiles=[]}const u=[...window._ptGaleriaUrls||[]],c=document.getElementById("ptPublicarTienda")?.checked??!1,f=document.getElementById("ptDescripcionWeb")?.value?.trim()||"",b=(window.categories||[]).find(a=>a.id===i),E=o||generateSKU(i),M=[...window._ptTagsActuales||[]],T=[...window._ptMpComponentes||[]];if(window.currentVariants=[...window._ptVariants||[]],window.modoEdicion&&window.edicionProductoId!==null){const a=(window.products||[]).findIndex(_=>String(_.id)===String(window.edicionProductoId));if(a===-1){manekiToastExport("Error: producto no encontrado","err");return}const y=window.products[a].stock,w=window.products[a],v=w.price,z=w.cost,S=w.historialPrecios||[];(v!==r||z!==g)&&S.push({fecha:new Date().toISOString(),precioAntes:v,costoAntes:z,precioNuevo:r,costoNuevo:g}),window.products[a]=Object.assign({},window.products[a],{name:t,category:i,tipo:c?"producto":"producto_interno",cost:g,price:r,stockMin:s,tags:M,sku:E,mpComponentes:T,publicarTienda:c,descripcionWeb:f,image:b?b.emoji:window.products[a].image,imageUrl:window.currentProductImage||window.products[a].imageUrl,imageUrls:u.length>0?u:window.products[a].imageUrls||[],variants:[...window.currentVariants],historialPrecios:S,rendimientoPorHoja:d,proveedorNombre:l,proveedorNotas:m,movimientos:window.products[a].movimientos||[]}),syncStockFromVariants(window.products[a]);const P=getStockEfectivo(window.products[a]);P!==y&&registrarMovimiento({productoId:window.edicionProductoId,productoNombre:t,tipo:"ajuste",cantidad:P-y,motivo:"Edici\xF3n",stockAntes:y,stockDespues:P});const $=P-y;$!==0&&(window.products[a].movimientos=window.products[a].movimientos||[],window.products[a].movimientos.unshift({id:Date.now(),fecha:_fechaHoy(),delta:$,stockResultante:P,motivo:"Edici\xF3n manual",usuario:"local"}),window.products[a].movimientos.length>30&&(window.products[a].movimientos=window.products[a].movimientos.slice(0,30))),saveProducts(),renderInventoryTable(),typeof updateDashboard=="function"&&updateDashboard(),C(!0),closePtModal(),window.MKS&&MKS.notify(),manekiToastExport("\u2705 Producto actualizado","ok")}else{const a={id:_genId(),name:t,category:i,tipo:c?"producto":"producto_interno",cost:g,price:r,stock:0,stockMin:s,tags:M,sku:E,mpComponentes:T,publicarTienda:c,descripcionWeb:f,image:b?b.emoji:"\u{1F4E6}",imageUrl:window.currentProductImage||null,imageUrls:u,variants:[...window.currentVariants],rendimientoPorHoja:d,proveedorNombre:l,proveedorNotas:m,movimientos:[]};syncStockFromVariants(a),window.products.push(a),saveProducts(),renderInventoryTable(),typeof updateDashboard=="function"&&updateDashboard(),C(!0),closePtModal(),window.MKS&&MKS.notify(),manekiToastExport("\u2705 Producto agregado exitosamente","ok")}}finally{k&&(k.disabled=!1)}}window.guardarProductoTerminado=guardarProductoTerminado;
