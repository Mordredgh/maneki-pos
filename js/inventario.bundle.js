            ...
            <td class="px-2 py-3">
                <div style="display:flex;gap:3px;flex-wrap:wrap;">
                    ...
                    <button type="button" onclick="duplicarProducto('${f}')" title="Duplicar" aria-label="Duplicar producto"
                        style="width:28px;height:28px;border-radius:7px;border:1px solid rgba(150,105,196,0.2);background:rgba(150,105,196,0.08);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:13px;">\u{1F4CB}</button>
        ...
        <div style="margin-bottom:32px;border-radius:16px;overflow:hidden;border:1.5px solid ${f}33;box-shadow:0 2px 12px ${f}11;">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 20px;background:${w};border-bottom:${ce?"none":"1.5px solid "+f+"33"};cursor:pointer;" onclick="(()=>{const k='_invSec_${t}_collapsed';window[k]=!window[k];renderInventoryTable()})()">
                <div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <span style="background:${f};color:#fff;font-size:11px;font-weight:700;padding:2px 10px;border-radius:99px;">${C}</span>
                </div>
...
`;b.forEach(L=>{const u=[new Date(L.fecha).toLocaleString("es-MX"),L.productoNombre||"",L.tipo||"",L.cantidad,L.motivo||"",L.stockAntes??"",L.stockDespues??""];F+=u.map(h=>`"${String(h).replace(/"/g,'""')}"`).join(",")+`
