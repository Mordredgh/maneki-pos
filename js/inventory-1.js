  ...
  if (resultado.length === 0) {
    ...
    if (faltantes.length > 0) {
      ...
      faltantes.forEach((r) => {
        ...
        const buyBtn = provUrl ? `<a href="${_esc(provUrl)}" target="_blank" style="display:inline-block;margin-top:3px;font-size:.65rem;padding:2px 6px;border-radius:999px;background:#dbeafe;color:#1d4ed8;text-decoration:none;font-weight:700;">🛒 Comprar</a>` : "";
...
async function _notificarStockTelegram({ agotados, bajos, mpAgotadas, mpBajas }) {
  ...
  if (!BOT_TOKEN) {
    console.warn("Telegram: telegramBotToken no configurado, omitiendo notificación de stock.");
    return;
  }
...
function confirmarStockInline(id) {
  ...
  if (isNaN(nuevo) || nuevo < 0) {
    manekiToastExport("Stock inválido", "err");
    return;
  }
...
function poblarFiltroProveedores() {
  ...
  sel.innerHTML = '<option value="">🏪 Todos los proveedores</option>' + proveedores.map((pr) => `<option value="${_esc(pr)}"${current === pr ? "selected" : ""}>${_esc(pr)}</option>`).join("");
}
...
async function registrarMerma(id) {
  ...
  if (!cant || cant <= 0) {
    manekiToastExport("⚠️ Cantidad inválida", "warn");
    return;
  }
...
function ptAgregarFotosGaleria(fileList) {
  ...
  if (disponibles <= 0) {
    manekiToastExport(`Ya tienes ${MAX} fotos en la galería`, "warn");
    return;
  }
...
async function importarInventarioCSV(input) {
  ...
  if (lines.length < 2) {
    manekiToastExport("⚠️ CSV vacío o sin datos", "warn");
    return;
  }
...
function verSnapshotsInventario() {
  ...
  if (!snaps.length) {
    manekiToastExport("Sin snapshots guardados aún", "warn");
    return;
  }
