...
function dibujarAnillosEnMapa(mapa, readOnly) {
}
function limpiarCirculos() {
  if (_mapaAnillosLayerGroup) _mapaAnillosLayerGroup.clearLayers();
}
...
function buscarNominatim(q) {
  _geocodeFetch(q + ", Monterrey, Nuevo León", 6).then((r) => {
...
⚠️ _La tarifa puede variar según condiciones del envío_`;
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(texto).then(() => manekiToastExport("✅ Copiado al portapapeles", "ok")).catch(() => _copiarFallback(texto));
  } else {
...
function _copiarFallback(texto) {
  ...
  try {
    ...
    manekiToastExport("✅ Copiado al portapapeles", "ok");
  } catch (e) {
    manekiToastExport("No se pudo copiar, cópialo manualmente", "warn");
  }
...
function buscarDestinoCotizador() {
  ...
  if (!dir) {
    manekiToastExport("Escribe una dirección para buscar", "warn");
    return;
  }
  buscarNominatim(dir);
  _geocodeFetch(dir + ", Monterrey, Nuevo León", 1).then((r) => {
    if (!r.ok) throw new Error("nominatim " + r.status);
    return r.json();
  }).then((data) => {
    if (data.length) {
...
function renderTablaAnillos() {
  ...
  const sorted = [...envioAnillos].sort((a, b) => a.km - b.km);
  if (!sorted.length) {
    el.innerHTML = '<p class="text-xs text-gray-400 text-center py-3">Sin anillos configurados</p>';
    return;
  }
...
function initMapaConfigLeaflet() {
  ...
  L.marker([ENVIO_BASE.lat, ENVIO_BASE.lng], { icon: baseIcon }).addTo(_mapaConfig).bindPopup(`<b>📍 Tu base</b><br>${storeConfig.address || "Base configurada"}`).openPopup();
...
function redrawMapaConfig() {
  ...
  sorted.forEach((anillo, i) => {
    ...
    L.circle([ENVIO_BASE.lat, ENVIO_BASE.lng], {
      ...
      fillColor: color,
      fillOpacity: 0.08
    }).addTo(_mapaConfigLayerGroup).bindTooltip(`<b>${anillo.label}</b> — $${anillo.precio}`, { sticky: true });
  });
}
(function() {
  function _safeExp(name) {
    ...
    const local = {
      switchQuoteTab,
      buscarDestinoCotizador,
      seleccionarNominatim,
      copiarCotizacionEnvio,
      abrirConfigAnillos,
      cerrarConfigAnillos,
      guardarAnillos,
      agregarAnillo,
      eliminarAnilloByIndex,
      updateAnilloByIndex,
      redrawMapaConfig,
      renderTablaAnillos
    };
  ...
  const names = [
    ...
    "closePedidoModal",
    "setPedidoStatus",
    "openPedidoStatusModal",
    "closePedidoStatusModal",
    "openCancelPedidoModal",
    "closeCancelPedidoModal",
    "confirmarCancelPedido",
    "confirmarAbonoPedido",
    "cerrarAbonoPedido",
    "openAbonoPedido",
    "selectAbonoPedidoMethod",
    "filterPedidos",
    "setVistaPedidos",
    ...
    "agregarTagCustom",
    "eliminarTag",
    "toggleTagPredefinido",
    "eliminarPedido",
    "eliminarPedidoFinalizado",
    "filtrarProduccion",
    "imprimirListaProduccion",
    "toggleListaProduccion",
    // Ventas / Análisis
    "abrirDetalleSale",
    "abrirDetalleSaleById",
    "cerrarDetalleSale",
