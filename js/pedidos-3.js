async function imprimirTicketPedido(i){const e=[...window.pedidosFinalizados||[],...window.pedidos||[]].find(x=>String(x.id)===String(i));if(!e)return;let o="";try{const x=new URL("logo.png",window.location.href).href,w=await(await fetch(x)).blob();o=await new Promise(P=>{const h=new FileReader;h.onload=()=>P(h.result),h.readAsDataURL(w)})}catch{}const t=Number(e.total||0),a=Number(e.anticipo||0),s=(e.pagos||[]).reduce((x,v)=>x+Number(v.monto||0),0),g=s>0?s:Number(e.anticipo||0),c=Math.max(0,Number(e.total||0)-g),r=(e.fechaFinalizado||e.fechaPedido||"").split("T")[0]||"\u2014",p=e.entrega||"\u2014",n=(e.productosInventario||[]).filter(x=>x.id!=="libre"),l=n.length>0?n.map(x=>{const v=Number(x.quantity||1),w=Number(x.price||0),P=v*w;let h="";if(x.variante){const y=x.variante.indexOf(":"),$=y!==-1?x.variante.slice(0,y).trim():"",E=y!==-1?x.variante.slice(y+1).trim():x.variante.trim();h=`<div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:3px;">
                ...
                </div>`}const S=w>0?`$${w.toFixed(2)}`:'<span style="color:#d1d5db;">\u2014</span>',k=w>0?`$${P.toFixed(2)}`:'<span style="color:#d1d5db;">\u2014</span>';return`
            <tr>
                ...
                <td style="padding:10px 12px;border-bottom:1px solid #f3f4f6;text-align:center;color:#6b7280;font-size:13px;vertical-align:middle;">${v}</td>
  ...
  <div class="actions">
    ...
    <button class="btn btn-close" onclick="window.close()">\u2715 Cerrar</button>
        ...
        <div class="flex items-center gap-2 px-3 py-2 bg-purple-50 border border-purple-100 rounded-xl text-sm">
            <div class="flex-1 min-w-0">
                <div class="font-medium text-gray-800 truncate">${t.name}${t.variante?` <span class="text-xs text-purple-600 font-semibold">(${(()=>{const c=t.variante.indexOf(":");if(c===-1)return t.variante;const r=t.variante.slice(0,c).trim(),p=t.variante.slice(c+1).trim();return r+": "+(typeof _mkColorDot=="function"?_mkColorDot(r,p):p)})()})</span>`:""}</div>
