"use strict";async function imprimirTicketPedido(n){const e=[...window.pedidosFinalizados||[],...window.pedidos||[]].find(m=>String(m.id)===String(n));if(!e)return;let o="";try{const m=new URL("logo.png",window.location.href).href,v=await(await fetch(m)).blob();o=await new Promise(h=>{const w=new FileReader;w.onload=()=>h(w.result),w.readAsDataURL(v)})}catch{}const t=Number(e.total||0),d=Number(e.anticipo||0),s=(e.pagos||[]).reduce((m,b)=>m+Number(b.monto||0),0),g=s>0?s:Number(e.anticipo||0),l=Math.max(0,Number(e.total||0)-g),a=(e.fechaFinalizado||e.fechaPedido||"").split("T")[0]||"\u2014",p=e.entrega||"\u2014",i=(e.productosInventario||[]).filter(m=>m.id!=="libre"),c=i.length>0?i.map(m=>{const b=Number(m.quantity||1),v=Number(m.price||0),h=b*v;let w="";if(m.variante){const y=m.variante.indexOf(":"),P=y!==-1?m.variante.slice(0,y).trim():"",S=y!==-1?m.variante.slice(y+1).trim():m.variante.trim();w=`<div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:3px;">
                    ...
                    <span style="background:#fffbeb;color:#92400e;font-size:9px;font-weight:700;padding:1px 6px;border-radius:99px;text-transform:uppercase;">${S}</span>
                </div>`}const $=v>0?`$${v.toFixed(2)}`:'<span style="color:#d1d5db;">\u2014</span>',k=v>0?`$${h.toFixed(2)}`:'<span style="color:#d1d5db;">\u2014</span>';return`
            <tr>
            ...
            </tr>`}).join(""):`<tr><td colspan="4" style="padding:16px 12px;text-align:center;color:#9ca3af;font-style:italic;font-size:13px;">${e.concepto||"Pedido personalizado"}</td></tr>`,f=o?`<img src="${o}" style="height:72px;object-fit:contain;margin-bottom:8px;" alt="Maneki Store">`:'<div style="font-size:2rem;">\u{1F431}</div>',u=e.notas?`
