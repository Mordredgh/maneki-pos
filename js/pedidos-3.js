async function imprimirTicketPedido(i){const e=[...window.pedidosFinalizados||[],...window.pedidos||[]].find(x=>String(x.id)===String(i));if(!e)return;let o="";try{const x=new URL("logo.png",window.location.href).href,v=await(await fetch(x)).blob();o=await new Promise(P=>{const h=new FileReader;h.onload=()=>P(h.result),h.readAsDataURL(v)})}catch{}const t=Number(e.total||0),a=Number(e.anticipo||0),s=(e.pagos||[]).reduce((x,w)=>x+Number(w.monto||0),0),g=s>0?s:Number(e.anticipo||0),f=Math.max(0,Number(e.total||0)-g),r=(e.fechaFinalizado||e.fechaPedido||"").split("T")[0]||"\u2014",c=e.entrega||"\u2014",n=(e.productosInventario||[]).filter(x=>x.id!=="libre"),l=n.length>0?n.map(x=>{const w=Number(x.quantity||1),v=Number(x.price||0),P=w*v;let h="";if(x.variante){const y=x.variante.indexOf(":"),$=y!==-1?x.variante.slice(0,y).trim():"",E=y!==-1?x.variante.slice(y+1).trim():x.variante.trim();h=`<div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:3px;">
            ...
            </tr>`}).join(""):`<tr><td colspan="4" style="padding:16px 12px;text-align:center;color:#9ca3af;font-style:italic;font-size:13px;">${e.concepto||"Pedido personalizado"}</td></tr>`,p=o?`<img src="${o}" style="height:72px;object-fit:contain;margin-bottom:8px;" alt="Maneki Store">`:'<div style="font-size:2rem;">\u{1F431}</div>',u=typeof window._esc=="function"?window._esc:(x=>String(x||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")),d=e.notas?`
...
<div class="ticket">
  ...
  <div class="info-block">
    ...
    <div class="info-grid">
      ...
      <div class="info-cell">
        ...
        <div class="info-value" style="color:${c&&c!=="\u2014"&&r&&r!=="\u2014"&&c<r?"#dc2626":"#1f2937"};">${c}</div>
      </div>
      ${e.concepto?`
      <div class="info-cell full">
        <div class="info-label">Concepto</div>
        <div class="info-value" style="font-weight:500;font-size:12px;">${e.concepto}</div>
      </div>`:""}
    </div>
  </div>
  ...
  <div class="totals">
    ...
    <div class="total-row ${f<=0?"saldo-ok":"saldo-pen"}">
      <span>${f<=0?"\u2705 Pagado completo":"\u23F3 Saldo pendiente"}</span>
      <span>$${Math.max(0,f).toFixed(2)}</span>
    </div>
  </div>
  ...
  <div class="actions">
    ...
    <button class="btn btn-close" onclick="window.close()">\u2715 Cerrar</button>
            ...
            </tr>`}).join(""):`<tr><td colspan="4" style="padding:16px;text-align:center;color:#9ca3af;font-style:italic;">${n(e.concepto||"Pedido personalizado")}</td></tr>`,p=(e.pagos||[]).length>0?`<div style="margin-top:16px;"><h4 style="font-size:12px;font-weight:700;color:#6b7280;text-transform:uppercase;margin-bottom:8px;">Historial de pagos</h4>
        ...
        <div style="background:linear-gradient(135deg,#1a0533,#2d0a4e);padding:28px 24px;text-align:center;color:white;border-radius:12px 12px 0 0;">
            ...
            ${c?`<div style="font-size:11px;color:rgba(255,255,255,.6);margin-top:4px;">${n(c)}</div>`:""}
        </div>
            ...
            <div onclick="seleccionarProductoPedido('${t.id}')"
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-1 flex-wrap">
                        <span class="font-semibold text-sm text-gray-800 truncate">${t.name}</span>
                        ${c}
                    </div>
