...
async function exportarPedidoPDF(id) {
  const p = [...window.pedidosFinalizados || [], ...window.pedidos || []].find((x) => String(x.id) === String(id));
  if (!p) {
    manekiToastExport("Pedido no encontrado", "warn");
    return;
  }
  ...
  try {
    await html2pdf().set({
      margin: 0,
      filename: `${p.folio || "pedido"}_${_e(p.cliente || "").replace(/\s+/g, "_")}.pdf`,
...
function filtrarProductosPedido() {
  ...
  if (!grid) return;
  const productos = (window.products || []).filter(
    (p) => (!p.tipo || p.tipo === "producto" || p.tipo === "producto_interno" || p.tipo === "pack" || p.tipo === "producto_variable") && (!q || (p.name || "").toLowerCase().includes(q) || (p.sku || "").toLowerCase().includes(q))
  );
  ...
  if (varRow && varSel) {
    ...
    if (_effVariants.length > 0) {
      varSel.innerHTML = _effVariants.map((v) => {
        const stockLabel = v.qty !== void 0 && v.qty !== null ? ` (${v.qty} pzs)` : "";
...
function _pvMostrarHint(p, qty) {
  ...
  for (const r of tabla) {
    if (qty >= r.cantidadMin) rangoActivo = r;
    else break;
  }
...
function agregarProductoPedido() {
  ...
  if (existe) {
    existe.quantity = (existe.quantity || 1) + qty;
  } else {
    window.pedidoProductosSeleccionados.push({ id, name: p.name, price: precioFinal, quantity: qty, variante });
  }
  if (p.tipo === "producto_variable") {
  ...
  list.innerHTML = items.map((item, i) => {
    ...
    return `
        <div class="flex items-center gap-2 px-3 py-2 bg-purple-50 border border-purple-100 rounded-xl text-sm">
            <div class="flex-1 min-w-0">
                <div class="font-medium text-gray-800 truncate">${item.name}${item.variante ? ` <span class="text-xs text-purple-600 font-semibold">(${(() => {
      const p = item.variante.indexOf(":");
      if (p === -1) return item.variante;
      const t = item.variante.slice(0, p).trim(), val = item.variante.slice(p + 1).trim();
...
function _mostrarGaleriaPtEnPedido(prod) {
  ...
  const urls = Array.isArray(prod.imageUrls) && prod.imageUrls.length > 0 ? prod.imageUrls : prod.imageUrl ? [prod.imageUrl] : [];
  if (urls.length === 0) {
    galDiv.innerHTML = "";
    return;
  }
...
let _calAnio = (/* @__PURE__ */ new Date()).getFullYear();
...
function checkAlertasCobro() {
  ...
  const pendientes = pedidos.filter((p) => {
    ...
    const da = /* @__PURE__ */ new Date(a.entrega + "T00:00:00");
    const db = /* @__PURE__ */ new Date(b.entrega + "T00:00:00");
    return da - db;
  });
...
function imprimirEtiquetaPedido(id) {
  const p = (window.pedidos || []).find((x) => x.id === id) || (window.pedidosFinalizados || []).find((x) => x.id === id);
  if (!p) return;
...
async function verificarEntregasProximas({ silencioso = false } = {}) {
  ...
  if (!BOT_TOKEN) {
    console.debug("Telegram: telegramBotToken no configurado, omitiendo recordatorio de entregas.");
    return;
  }
  const chatIds = [cfg.telegramChatId1, cfg.telegramChatId2].filter(Boolean);
  const hoy = /* @__PURE__ */ new Date();
  hoy.setHours(0, 0, 0, 0);
  const en24 = new Date(hoy);
  en24.setDate(en24.getDate() + 1);
  const en48 = new Date(hoy);
  en48.setDate(en48.getDate() + 2);
...
${lineas.join("\n")}`;
  for (const chatId of chatIds) {
    try {
      ...
      console.warn("Telegram entrega reminder error:", e);
    }
  }
}
window.verificarEntregasProximas = verificarEntregasProximas;
(function initRecordatorioEntregas() {
  setTimeout(() => verificarEntregasProximas({ silencioso: true }), 8e3);
  setInterval(() => verificarEntregasProximas({ silencioso: true }), 12 * 60 * 60 * 1e3);
})();
