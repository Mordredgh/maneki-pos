(function() {
  ...
  console.log("%c\u{1F431} Maneki Store Premium v4.1", "color:#E8B84B;font-size:14px;font-weight:800;");
  ...
  function _setupSidebarAccordion() {
    ...
    toggle.innerHTML = '<span class="chevron">\u25BC</span> M\xE1s herramientas';
    toggle.type = "button";
  ...
  const _TIMELINE_STEPS = [
    ...
    { key: "produccion", label: "Producci\xF3n", icon: "\u{1F527}" },
    { key: "envio", label: "Env\xEDo", icon: "\u{1F4E6}" },
    { key: "salida", label: "Salida", icon: "\u{1F69A}" },
    { key: "retirar", label: "Retirar", icon: "\u{1F3EA}" }
  ];
  ...
  (function _onboardingTour() {
    ...
    setTimeout(() => {
      const steps = [
        { target: '[data-section="pedidos"]', title: "\u{1F4CB} Pedidos por Encargo", text: "Aqu\xED gestionas todos tus pedidos. Crea nuevos, da seguimiento y cobra." },
        { target: '[data-section="inventory"]', title: "\u{1F4E6} Inventario", text: "Controla tus productos, materias primas y stock en tiempo real." },
        { target: '[data-section="balance"]', title: "\u{1F4B0} Balance", text: "Registra ingresos, gastos y ve cu\xE1nto te deben tus clientes." },
        { target: '[data-section="clientes"]', title: "\u{1F465} Clientes", text: "Tu base de datos de clientes con historial de pedidos y estad\xEDsticas." },
        { target: "#mk-help-fab", title: "\u2328\uFE0F Atajos", text: "Presiona ? en cualquier momento para ver los atajos de teclado. \xA1Bienvenido a Maneki!" }
      ];
      let current = 0;
      const ov = document.createElement("div");
  ...
  (function _changelog() {
    ...
    const changes = [
      { icon: "\u2328\uFE0F", text: "Navegaci\xF3n r\xE1pida con teclas 1-9 para ir a cualquier secci\xF3n" },
      { icon: "\u{1F4CA}", text: "Sugerencia inteligente de reorden al ajustar stock (basada en velocidad de venta)" },
      { icon: "\u{1F4E6}", text: "Backup comprimido (.gz) \u2014 reduce el tama\xF1o hasta 80%" },
      { icon: "\u26A1", text: "Sync offline mejorado \u2014 sincroniza en lotes paralelos al reconectar" },
      { icon: "\u{1F4CB}", text: "Este changelog \u2014 te avisaremos cada vez que haya novedades" }
    ];
    setTimeout(() => {
      ...
      modal.addEventListener("click", (e) => {
        if (e.target === modal) {
          localStorage.setItem(KEY, MK_VERSION);
          modal.remove();
        }
      });
