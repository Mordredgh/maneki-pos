...
`;
  ...
  function initParticles() {
    ...
    const pts = Array.from({ length: 26 }, (_, i) => ({
      ...
      vx: (Math.random() - 0.5) * 35e-5,
      vy: (Math.random() - 0.5) * 35e-5,
    ...
    function draw(t) {
      ...
      pts.forEach((p, i) => {
        p.x = (p.x + p.vx + 1) % 1;
        p.y = (p.y + p.vy + 1) % 1;
        ctx.globalAlpha = 0.35 + 0.3 * Math.sin(t * 1e-3 + p.phase);
  ...
  function initConfetti() {
    ...
    window.mkConfetti = function() {
      if (window._mkConfettiRaf) {
        cancelAnimationFrame(window._mkConfettiRaf);
        window._mkConfettiRaf = null;
      }
  ...
  const MKS = {
    ...
    _play(notes, type) {
      ...
      const run = () => notes.forEach(([f, s, d, v = 0.28]) => {
        ...
        g.gain.exponentialRampToValueAtTime(1e-3, t + s + d);
        o.start(t + s);
        o.stop(t + s + d + 0.05);
      });
    ...
    tick() {
      this._tone([[880, 0, 0.08, 0.22], [1100, 0.06, 0.06, 0.15]]);
    },
    sale() {
      this._tone([[523, 0, 0.12, 0.28], [659, 0.1, 0.14, 0.28], [784, 0.22, 0.32, 0.28]], "triangle");
    },
    error() {
      this._tone([[220, 0, 0.16, 0.28], [190, 0.1, 0.12, 0.22]]);
    },
    notify() {
      this._tone([[880, 0, 0.08, 0.25], [1108, 0.09, 0.09, 0.22]]);
    },
    del() {
      this._tone([[440, 0, 0.08, 0.22], [330, 0.06, 0.1, 0.18]]);
    },
    toggle() {
  ...
  const SECTION_PALETTE = {
    dashboard: "rgba(124,58,237,0.10)",
    bienvenida: "rgba(124,58,237,0.10)",
    pedidos: "rgba(139,92,246,0.09)",
    inventory: "rgba(16,185,129,0.09)",
    balance: "rgba(245,158,11,0.09)",
    clientes: "rgba(255,107,157,0.09)",
    analisis: "rgba(197,151,59,0.09)",
    reportes: "rgba(99,102,241,0.09)",
    configuracion: "rgba(139,92,246,0.09)",
    categorias: "rgba(249,115,22,0.09)",
    equipos: "rgba(100,116,139,0.09)",
  ...
  function _countUp(el, target, prefix, duration) {
    ...
    const fmt = (v) => prefix === "$" ? "$" + v.toLocaleString("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : v.toLocaleString("es-MX", { maximumFractionDigits: 0 });
    (function frame(now) {
  ...
  function _animateKPIs() {
    const ids = [
      { id: "dailySales", prefix: "$" },
      { id: "netProfit", prefix: "$" },
      { id: "accountsReceivable", prefix: "$" },
      { id: "dashActivePedidos", prefix: "" }
    ];
  ...
  function _initEditableGoal() {
    ...
    if (pct && !document.getElementById("mk-goal-edit")) {
      ...
      btn.id = "mk-goal-edit";
      btn.className = "mk-goal-edit-btn";
      btn.type = "button";
...
`;
  ...
  const _lazySections = /* @__PURE__ */ new Set();
  function _sectionSpinner(name, show) {
    const sectionId = "section-" + name;
    const el = document.getElementById(sectionId) || document.querySelector('[data-section="' + name + '"]');
    if (!el) return;
      ...
      window.renderAnalisis = function() {
        ...
        wk.onmessage = function(ev) {
          ...
          [
            ...
            ["analisisKpiGanancia", kpis.profit, "$"]
          ].forEach(([id, val, pre]) => {
            const el = document.getElementById(id);
          ...
          if (typeof window.renderAnalisisABC === "function") {
            ...
            var totalGanancia = rows.reduce(function(s, r) {
              return s + r.profit;
            }, 0);
        ...
        (window.products || []).forEach(function(p) {
          if (p.tipo === "materia_prima" || p.tipo === "servicio") {
            if (p.name) _mpSet[p.name] = 1;
            if (p.id) _mpSet["id:" + String(p.id)] = 1;
          }
        });
        wk.postMessage({ salesHistory: window.salesHistory || [], pedidosFinalizados: window.pedidosFinalizados || [], period, orden, desde, hasta, mpSet: _mpSet });
      };
  ...
  function _patchSoundActions() {
    ...
    if (!window.manekiToastExport._mk4) {
      ...
      window.manekiToastExport = function(m, t) {
        _o(m, t);
        if (t === "err") MKS.error();
      };
  ...
  const _mkPatchFallback = setInterval(() => {
    ...
    _patchSoundActions();
    const allDone = window.showSection?._mk4 && window.renderAnalisis?._mk4 && window.manekiToastExport?._mk4;
  ...
  console.log(
    "%c🐱 Maneki Premium v4.1 ·",
    "color:#E8B84B;font-weight:800;font-size:12px;",
