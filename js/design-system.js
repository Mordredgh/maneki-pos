...
`,b=document.createElement("style");b.id="mk-premium-v4",b.textContent=M,document.head.appendChild(b);function D(){if(document.getElementById("mk-particles"))return;const e=document.createElement("canvas");e.id="mk-particles",document.body.insertBefore(e,document.body.firstChild);const t=e.getContext("2d");let o,i;function c(){o=e.width=innerWidth,i=e.height=innerHeight}c(),addEventListener("resize",c);const r=["rgba(255,209,102,OP)","rgba(255,221,133,OP)","rgba(150,105,196,OP)","rgba(171,132,209,OP)"],l=Array.from({length:26},(d,s)=>({x:Math.random(),y:Math.random(),r:Math.random()*2+.7,color:r[s%r.length].replace("OP",(Math.random()*.22+.04).toFixed(2)),vx:(Math.random()-.5)*35e-5,vy:(Math.random()-.5)*35e-5,phase:Math.random()*Math.PI*2}));let a;function n(d){t.clearRect(0,0,o,i),l.forEach((s,L)=>{s.x=(s.x+s.vx+1)%1,s.y=(s.y+s.vy+1)%1,t.globalAlpha=.35+.3*Math.sin(d*.001+s.phase),t.fillStyle=s.color,t.beginPath(),t.arc(s.x*o,s.y*i,s.r,0,Math.PI*2),t.fill()}),t.globalAlpha=1,a=requestAnimationFrame(n)}n(0),document.addEventListener("visibilitychange",()=>{document.hidden?cancelAnimationFrame(a):a=requestAnimationFrame(n)})}D();function z(){if(document.getElementById("mk-confetti"))return;const e=document.createElement("canvas");e.id="mk-confetti",document.body.appendChild(e);const t=e.getContext("2d");function o(){e.width=innerWidth,e.height=innerHeight}o(),addEventListener("resize",o);const i=["#FFDD85","#FFE8B0","#9669c4","#ab84d1","#10B981","#FF6B9D","#fff"];let c=[],r;window.mkConfetti=function(){window._mkConfettiRaf&&(cancelAnimationFrame(window._mkConfettiRaf),window._mkConfettiRaf=null),e.style.display="block",c=[];for(let l=0;l<88;l++)c.push({x:e.width/2+(Math.random()-.5)*280,y:e.height*.42,vx:(Math.random()-.5)*11,vy:-(Math.random()*13+5),r:Math.random()*5+2.5,rot:Math.random()*360,rotV:(Math.random()-.5)*9,color:i[Math.floor(Math.random()*i.length)],life:1,decay:Math.random()*.017+.009});cancelAnimationFrame(r),(function l(){t.clearRect(0,0,e.width,e.height);let a=!1;c.forEach(n=>{n.x+=n.vx,n.y+=n.vy,n.vy+=.44,n.vx*=.995,n.rot+=n.rotV,n.life-=n.decay,n.life>0&&(a=!0),t.save(),t.globalAlpha=Math.max(0,n.life),t.translate(n.x,n.y),t.rotate(n.rot*Math.PI/180),t.fillStyle=n.color,t.fillRect(-n.r,-n.r/2,n.r*2,n.r),t.restore()}),a?(r=requestAnimationFrame(l),window._mkConfettiRaf=r):(window._mkConfettiRaf=null,t.clearRect(0,0,e.width,e.height),e.style.display="none")})()}}z();const u={_ctx:null,muted:localStorage.getItem("mk_muted")==="1",_ac(){if(!this._ctx)try{this._ctx=new(AudioContext||webkitAudioContext)}catch{}return this._ctx},_play(e,t){const o=this._ac();if(!o)return;const i=()=>e.forEach(([c,r,l,a=.28])=>{const n=o.createOscillator(),d=o.createGain();n.connect(d),d.connect(o.destination),n.type=t,n.frequency.value=c;const s=o.currentTime;d.gain.setValueAtTime(0,s+r),d.gain.linearRampToValueAtTime(a,s+r+.012),d.gain.exponentialRampToValueAtTime(.001,s+r+l),n.start(s+r),n.stop(s+r+l+.05)});o.state==="suspended"?o.resume().then(i):i()},_tone(e,t="sine"){this.muted||this._play(e,t)},tick(){this._tone([[880,0,.08,.22],[1100,.06,.06,.15]])},sale(){this._tone([[523,0,.12,.28],[659,.1,.14,.28],[784,.22,.32,.28]],"triangle")},error(){this._tone([[220,0,.16,.28],[190,.1,.12,.22]])},notify(){this._tone([[880,0,.08,.25],[1108,.09,.09,.22]])},del(){this._tone([[440,0,.08,.22],[330,.06,.1,.18]])},toggle(){this.muted=!this.muted,localStorage.setItem("mk_muted",this.muted?"1":"0");const e=document.getElementById("mk-sound-btn");e&&(e.textContent=this.muted?"\u{1F507}":"\u{1F50A}",e.classList.toggle("muted",this.muted))}};if(window.MKS=u,!document.getElementById("mk-sound-btn")){const e=document.createElement("button");e.id="mk-sound-btn",e.title="Sonidos",e.textContent=u.muted?"\u{1F507}":"\u{1F50A}",u.muted&&e.classList.add("muted"),e.onclick=()=>u.toggle(),document.body.appendChild(e)}let p=document.getElementById("mk-morph-overlay");p||(p=document.createElement("div"),p.id="mk-morph-overlay",document.body.appendChild(p));const y={dashboard:"rgba(150,105,196,0.10)",bienvenida:"rgba(150,105,196,0.10)",pedidos:"rgba(139,92,246,0.09)",inventory:"rgba(16,185,129,0.09)",balance:"rgba(245,158,11,0.09)",clientes:"rgba(255,107,157,0.09)",analisis:"rgba(255,209,102,0.09)",reportes:"rgba(99,102,241,0.09)",configuracion:"rgba(139,92,246,0.09)",categorias:"rgba(249,115,22,0.09)",equipos:"rgba(100,116,139,0.09)",quotes:"rgba(255,209,102,0.09)"};let x="bienvenida";function B(e){const t=y[x]||"rgba(150,105,196,0.08)",o=y[e]||"rgba(150,105,196,0.08)";p.style.background=`linear-gradient(135deg,${t},${o})`,p.classList.remove("active"),requestAnimationFrame(()=>requestAnimationFrame(()=>{p.classList.add("active"),setTimeout(()=>p.classList.remove("active"),420)})),x=e}window._mkMorphTo=B;function k(e,t,o,i){if(!e||isNaN(t))return;const c=performance.now();e.classList.add("mk-count-pop");const r=l=>o==="$"?"$"+l.toLocaleString("es-MX",{minimumFractionDigits:2,maximumFractionDigits:2}):l.toLocaleString("es-MX",{maximumFractionDigits:0});(function l(a){const n=Math.min((a-c)/i,1),d=1-Math.pow(1-n,3);e.textContent=r(t*d),n<1?requestAnimationFrame(l):e.textContent=r(t)})(c)}function v(){[{id:"dailySales",prefix:"$"},{id:"netProfit",prefix:"$"},{id:"accountsReceivable",prefix:"$"},{id:"dashActivePedidos",prefix:""}].forEach(({id:t,prefix:o})=>{const i=document.getElementById(t);if(!i)return;const c=parseFloat((i.textContent||"").replace(/[^0-9.-]/g,""));!isNaN(c)&&c>0&&k(i,c,o,820)}),document.querySelectorAll(".mk-kpi-value").forEach(t=>{const o=parseFloat((t.textContent||"").replace(/[^0-9.-]/g,"")),i=(t.textContent||"").trim().startsWith("$")?"$":"";!isNaN(o)&&o>0&&k(t,o,i,820)})}window._mkAnimateKPIs=v;function _(){const e=document.getElementById("dashMonthGoal");if(!e)return;const t=localStorage.getItem("mk_monthly_goal");t&&!isNaN(parseFloat(t))&&(e.value=parseFloat(t));const o=document.getElementById("dashGoalPercent");if(o&&!document.getElementById("mk-goal-edit")){const i=document.createElement("button");i.id="mk-goal-edit",i.className="mk-goal-edit-btn",i.type="button",i.textContent="\u270E META",i.onclick=()=>{const c=i,r=document.getElementById("dashGoalPercent"),l=e.value||"5000",a=document.createElement("input");a.type="number",a.value=l,a.min="1",a.style.cssText="width:110px;padding:2px 6px;border:1.5px solid #FFDD85;border-radius:8px;font-size:0.85rem;outline:none;font-family:inherit;",i.style.display="none",i.parentNode.insertBefore(a,i.nextSibling),a.focus(),a.select();const n=()=>{const d=parseFloat(a.value);d>0&&(e.value=d,localStorage.setItem("mk_monthly_goal",d),window.updateDashboard&&window.updateDashboard(),window.manekiToastExport&&manekiToastExport("Meta mensual actualizada: $"+d.toLocaleString("es-MX"),"ok")),a.remove(),i.style.display=""};a.addEventListener("blur",n),a.addEventListener("keydown",d=>{d.key==="Enter"&&(d.preventDefault(),a.blur()),d.key==="Escape"&&(a.removeEventListener("blur",n),a.remove(),i.style.display="")})},o.parentNode.insertBefore(i,o.nextSibling)}}const R=`
...
`;let h=null;function O(){if(h)return h;try{const e=new Blob([R],{type:"application/javascript"});h=new Worker(URL.createObjectURL(e))}catch{return null}return h}const E=new Set;function S(e,t){const o="section-"+e,i=document.getElementById(o)||document.querySelector('[data-section="'+e+'"]');if(!i)return;const c="__mk_spin_"+e;if(t){if(i.querySelector("#"+c))return;const r=document.createElement("div");if(r.id=c,r.style.cssText="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.7);z-index:99;border-radius:12px;",r.innerHTML='<div style="width:36px;height:36px;border:3px solid #f3f3f3;border-top:3px solid #FFD166;border-radius:50%;animation:mkSpin .7s linear infinite;"></div>',!document.getElementById("mk-spin-style")){const l=document.createElement("style");l.id="mk-spin-style",l.textContent="@keyframes mkSpin{to{transform:rotate(360deg)}}",document.head.appendChild(l)}i.style.position=i.style.position||"relative",i.appendChild(r)}else{const r=i.querySelector("#"+c);r&&r.remove()}}function N(e){if(E.has(e))return;E.add(e);const t=async()=>{(e==="balance"||e==="reportes"||e==="analisis")&&window._mkEnsureChartJs&&await window._mkEnsureChartJs(),e==="analisis"&&window.renderAnalisis&&window.renderAnalisis(),e==="reportes"&&window.renderSalesHistory&&window.renderSalesHistory(),e==="clientes"&&window.renderClientsTable&&window.renderClientsTable(),e==="balance"&&window.renderBalance&&window.renderBalance(),e==="quotes"&&window.renderQuotesTable&&window.renderQuotesTable(),(e==="inventory"||e==="inventario"||e==="categorias")&&(window.updateCategorySelects&&window.updateCategorySelects(),window.renderInventoryTable&&window.renderInventoryTable()),e==="pedidos"&&(window.renderPedidosTable&&window.renderPedidosTable(),window.renderKanbanBoard&&window.renderKanbanBoard(),window.renderHistorialPedidos&&window.renderHistorialPedidos())};if(!window._mkLazyLoad||window._mkGrupoListo(e)){(window._mkLazyLoad?window._mkLazyLoad(e):Promise.resolve()).then(()=>setTimeout(t,80));return}S(e,!0),window._mkLazyLoad(e).then(()=>{S(e,!1),setTimeout(t,80)})}window._lazyLoad=N;function A(){const e=document.getElementById("inventorySearch");e&&!e._mkD&&(e._mkD=!0)}function C(){return window.showSection&&(window.showSection._mk4=!0),!0}function I(){if(window.renderAnalisis&&!window.renderAnalisis._mk4){const e=window.renderAnalisis;return window.renderAnalisis=function(){const t=O();if(!t||!window.salesHistory){e.apply(this,arguments);return}const o=document.getElementById("analisisTabla");o&&(o.innerHTML=Array(5).fill(`<tr>${Array(8).fill('<td class="px-6 py-3"><div class="mk-skeleton" style="height:13px;width:75%;"></div></td>').join("")}</tr>`).join(""));const i=window.analisisPeriodoActual||"mes",c=document.getElementById("analisisOrden")?.value||"unidades",r=document.getElementById("analisisDesde")?.value||"",l=document.getElementById("analisisHasta")?.value||"";t.onmessage=function(n){const{rows:d,kpis:s}=n.data;[["analisisKpiProductos",s.products,""],["analisisKpiUnidades",s.units,""],["analisisKpiIngresos",s.revenue,"$"],["analisisKpiGanancia",s.profit,"$"]].forEach(([m,f,g])=>{const w=document.getElementById(m);w&&(w.textContent=g?g+""+Number(f).toFixed(2):f)});const L=s.revenue||1;if(o&&(o.innerHTML=d.slice(0,120).map((m,f)=>{const g=(m.revenue/L*100).toFixed(1),w=m.margin>=40?"text-green-600":m.margin>=20?"text-amber-500":"text-red-500",V=K=>String(K||"").replace(/</g,"&lt;").replace(/>/g,"&gt;");return`<tr class="hover:bg-amber-50 transition-colors">
                        ...
                        <td class="px-6 py-3 text-center font-semibold ${w}">${m.margin.toFixed(0)}%</td>
                        <td class="px-6 py-3">
                        ...
                        </td></tr>`}).join("")),typeof window.renderAnalisisABC=="function"){var H=d.map(function(m){return{nombre:m.name,unidades:m.units,ingresos:m.revenue,costoTotal:m.cost,ganancia:m.profit,margen:m.margin,emoji:"\u{1F4E6}"}}),G=d.reduce(function(m,f){return m+f.profit},0);window.renderAnalisisABC(H,G)}var P=document.getElementById("analisisVacio");P&&P.classList[d.length===0?"remove":"add"]("hidden")},t.onerror=()=>e.apply(this,arguments);const a={};(window.products||[]).forEach(function(n){(n.tipo==="materia_prima"||n.tipo==="servicio")&&(n.name&&(a[n.name]=1),n.id&&(a["id:"+String(n.id)]=1))}),t.postMessage({salesHistory:window.salesHistory||[],pedidosFinalizados:window.pedidosFinalizados||[],period:i,orden:c,desde:r,hasta:l,mpSet:a})},window.renderAnalisis._mk4=!0,!0}return!1}function F(){if(!(window.manekiToastExport&&window.deleteProduct&&window.deleteClient))return!1;if(!window.manekiToastExport._mk4){const t=window.manekiToastExport;window.manekiToastExport=function(o,i){t(o,i),i==="err"&&u.error()},window.manekiToastExport._mk4=!0}if(!window.deleteProduct._mk4){const t=window.deleteProduct;window.deleteProduct=function(){u.del(),t.apply(this,arguments)},window.deleteProduct._mk4=!0}if(!window.deleteClient._mk4){const t=window.deleteClient;window.deleteClient=function(){u.del(),t.apply(this,arguments)},window.deleteClient._mk4=!0}return!0}document.addEventListener("maneki:ready",function(e){const t=e?.detail?.module||"";(t==="reportes"||t==="showSection")&&C(),(t==="analisis"||t==="renderAnalisis")&&I(),(t==="app"||t==="toast")&&F()});let $=0;const q=setInterval(()=>{C(),I(),F();const e=window.showSection?._mk4&&window.renderAnalisis?._mk4&&window.manekiToastExport?._mk4;(e||++$>30)&&(clearInterval(q),e&&document.dispatchEvent(new CustomEvent("maneki:ready",{detail:{module:"design-system"}})))},300);function T(){A(),_(),setTimeout(()=>{A(),_()},1200);const e=localStorage.getItem("maneki_activeSection");(!e||e==="dashboard"||e==="bienvenida")&&setTimeout(v,900),e==="inventory"&&window.patchInventoryButtons&&setTimeout(window.patchInventoryButtons,800)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",T):T(),console.log("%c\u{1F41B} Bicho Capricho Premium v4.1 \xB7","color:#FFDD85;font-weight:800;font-size:12px;","Glassmorphism \xB7 Sound \xB7 Particles \xB7 Workers \xB7 Morph \xB7 InvPatch \u2713")})();
