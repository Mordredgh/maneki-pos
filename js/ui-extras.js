...
function busquedaGlobal(query) {
  ...
  if (prods.length > 0) {
    ...
    prods.slice(0, 4).forEach((p) => {
      const img = p.imageUrl ? `<img src="${p.imageUrl}" class="w-8 h-8 rounded-lg object-cover flex-shrink-0" onerror="this.style.display='none'">` : `<span class="text-lg flex-shrink-0">${p.image || "📦"}</span>`;
    ...
    clis.slice(0, 4).forEach((c) => {
      ...
      html += `<div class="px-4 py-2 hover:bg-blue-50 cursor-pointer flex items-center gap-3"
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="text-xs text-gray-400">${c.phone || c.telefono || "Sin teléfono"} · ${ventas2} venta${ventas2 !== 1 ? "s" : ""} · ${pedidosCli} pedido${pedidosCli !== 1 ? "s" : ""}</div>
        </div>
...
function initErrorLog() {
  ...
  modal.querySelector("#errorLogCopyBtn").addEventListener("click", () => {
    navigator.clipboard.writeText(JSON.stringify(window._errorLog, null, 2)).then(() => manekiToastExport("📋 Log copiado al portapapeles", "ok")).catch(() => manekiToastExport("❌ No se pudo copiar", "err"));
  });
