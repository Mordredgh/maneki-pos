...
function _getAllVentas() {
  ...
  var foliosEnSH = {};
  sh.filter(function(s) {
    return s.type === "pedido";
  }).forEach(function(s) {
    if (s.folio) foliosEnSH[s.folio] = true;
  });
  var idsLegacyPedido = {};
  sh.forEach(function(s) {
    if (s.type && s.type !== "") return;
    var folioEnConcepto = (s.concept || s.concepto || "").match(/PE-\d+/);
    if (folioEnConcepto) {
      idsLegacyPedido[s.id] = true;
      foliosEnSH[folioEnConcepto[0]] = true;
      return;
    }
    var matchPedido = pf.find(function(p) {
      ...
      var conceptoMatch = !p.concepto || !s.concept || p.concepto === (s.concept || s.concepto || "");
      return p.cliente === s.customer && Number(p.total) === Number(s.total) && fechaPed === s.date && conceptoMatch && !foliosEnSH[p.folio];
    });
    if (matchPedido) {
      idsLegacyPedido[s.id] = true;
  ...
  var shFiltrado = sh.filter(function(s) {
    return !idsLegacyPedido[s.id];
  });
  var pfComoVentas = pf.filter(function(p) {
    return !foliosEnSH[p.folio];
  }).map(function(p) {
    return {
      id: p.id,
      type: "pedido",
...
function renderValorInventario() {
  ...
  if (el("invMargenPromedio")) el("invMargenPromedio").textContent = "Margen potencial promedio: " + margen + "%";
}
...
function initComparativaMeses() {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    meses.push(label);
    ventas.push(todasVentas.filter(function(s) {
      return s.date && s.date.startsWith(mesStr) && s.method !== "Cancelado" && s.metodo !== "cancelado";
    }).reduce(function(s, v) {
      return s + (Number(v.total) || 0);
    }, 0));
    gastos.push(todasGastos.filter(function(e) {
      return e.date && e.date.startsWith(mesStr);
    }).reduce(function(s, e) {
      return s + (e.amount || 0);
    }, 0));
    ventasAnioAnterior.push(todasVentas.filter(function(s) {
      return s.date && s.date.startsWith(mesStrAnio) && s.method !== "Cancelado" && s.metodo !== "cancelado";
    }).reduce(function(s, v) {
      return s + (Number(v.total) || 0);
    }, 0));
  }
...
function _renderTicketPromedioStats(meses, ventas, todasVentas, now) {
  ...
  var bestIdx = ventas.reduce(function(maxIdx, v, idx) {
    return v > ventas[maxIdx] ? idx : maxIdx;
  }, 0);
...
function initTopProductosChart() {
  ...
  const prodMap = {};
  _getAllVentas().filter(function(s) {
  ...
  const top10 = Object.entries(prodMap).sort(function(a, b) {
    return b[1].ingresos - a[1].ingresos;
  }).slice(0, 10);
  ...
  topProductosChart = new Chart(canvas, {
    ...
    options: {
      ...
      scales: { x: { beginAtZero: true, ticks: { callback: function(v) {
        return "$" + v.toLocaleString();
      } } } }
    }
  });
...
function initMargenCategoriaChart() {
  ...
  cats.forEach(function(cat) {
    ...
    if (margenProm >= 40) colores.push("#10B981");
    else if (margenProm >= 20) colores.push("#F59E0B");
    else colores.push("#EF4444");
  });
  if (labels.length === 0) {
    var catMap = {};
    ...
    Object.entries(catMap).forEach(function(entry) {
      ...
      var prom = arr.reduce(function(s, v) {
        return s + v;
      }, 0) / arr.length;
      labels.push(k);
...
function updateMonthlyStats() {
  ...
  const monthlySales = _allV.reduce(function(sum, s) {
    return sum + Number(s.total || 0);
  }, 0);
  const monthlyUnits = _allV.reduce(function(sum, s) {
    return sum + (s.products || []).reduce(function(a, p) {
      return a + Number(p.quantity || 1);
    }, 0);
  }, 0);
...
function updatePaymentMethods() {
  const _pmVentas = _getAllVentas().filter(function(s) {
    return s.type !== "abono" && s.type !== "anticipo";
  });
  const el = (id) => document.getElementById(id);
  const totalMonto = _pmVentas.reduce(function(s, v) {
    return s + Number(v.total || v.amount || v.monto || 0);
  }, 0);
  if (totalMonto === 0) {
  ...
  const montoMetodo = function(m) {
    return _pmVentas.filter(function(s) {
      return s.method === m || s.paymentMethod === m;
    }).reduce(function(s, v) {
      return s + Number(v.total || v.amount || v.monto || 0);
    }, 0);
  };
  const pct = function(m) {
    return (montoMetodo(m) / totalMonto * 100).toFixed(0) + "%";
  };
...
function renderTopProducts() {
  ...
  if (!el) return;
  const _todasVentas = _getAllVentas().filter(function(s) {
    return s.type !== "abono" && s.type !== "anticipo";
  });
  ...
  _todasVentas.forEach(function(sale) {
    (sale.products || []).forEach(function(item) {
      if (esMPOServicio(item)) return;
      var k2 = item.name || item.id || "?";
      if (!productSales[k2]) productSales[k2] = { name: item.name || k2, sales: 0, revenue: 0 };
      productSales[k2].sales += Number(item.quantity || 1);
      productSales[k2].revenue += Number(item.price || 0) * Number(item.quantity || 1);
    });
...
function initCategoryChart() {
  ...
  if (categoryChartMode === "inventario") {
    ...
    Object.entries(catMap).forEach(([n, v]) => {
      if (v > 0) {
        labels.push(n);
        data.push(v);
      }
...
function renderSalesHistory() {
  ...
  const filtered = allSales.filter((sale, i) => {
    ...
    return (!query || customer.includes(query) || folio.includes(query)) && (!fechaDesde || sale.date >= fechaDesde) && (!fechaHasta || sale.date <= fechaHasta);
  });
  ...
  const _canLoadMore = (window.salesHistory || []).length >= 1e3 && !window._salesHistoryAllLoaded;
  if (_canLoadMore) {
    if (!_loadMoreBtn) {
...
function initChart() {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const dateStr = _fl(date);
    last7Days.push(dateStr);
    salesByDay[dateStr] = 0;
  }
  const _idsContados = /* @__PURE__ */ new Set();
  (window.salesHistory || []).forEach((sale) => {
    if (salesByDay.hasOwnProperty(sale.date) && sale.method !== "Cancelado" && sale.type !== "anticipo" && sale.type !== "abono") {
...
function descargarReporteVentas() {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `Reporte_Ventas_Maneki_${typeof _fechaHoy === "function" ? _fechaHoy() : (/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv`;
  a.click();
...
function abrirDetalleSaleById(id) {
  ...
  if (idx === -1) {
    manekiToastExport("Venta no encontrada", "err");
    return;
  }
...
function _renderDetalleSale(sale, idx) {
  ...
  if (telBox) {
    if (sale.telefono) {
      telBox.classList.remove("hidden");
      el("detalleSaleTel")?.setAttribute && (el("detalleSaleTel").textContent = sale.telefono);
    } else telBox.classList.add("hidden");
  }
  if (redesBox) {
    if (sale.redes) {
      redesBox.classList.remove("hidden");
      if (el("detalleSaleRedes")) el("detalleSaleRedes").textContent = sale.redes;
  ...
  if (conceptoBox) {
    if (conceptoTxt) {
      ...
      if (el("detalleSaleConcepto")) el("detalleSaleConcepto").textContent = conceptoTxt;
    } else conceptoBox.classList.add("hidden");
  }
  const prods = sale.products || [];
  if (el("detalleSaleProductos")) {
    el("detalleSaleProductos").innerHTML = prods.length > 0 ? prods.map((p) => {
    ...
    }).join("") : '<tr><td colspan="4" class="px-4 py-4 text-center text-gray-400 text-sm">Sin productos registrados</td></tr>';
  }
  ...
  if (descRow) {
    descRow.classList.toggle("hidden", !desc);
    if (desc && el("detalleSaleDesc")) el("detalleSaleDesc").textContent = "-$" + desc.toFixed(2);
  }
  if (taxRow) {
    taxRow.classList.toggle("hidden", !tax);
    if (tax && el("detalleSaleTax")) el("detalleSaleTax").textContent = "$" + tax.toFixed(2);
  }
...
function renderAnalisis() {
  ...
  ventasFiltradas.forEach((venta) => {
    (venta.products || []).forEach((item) => {
      ...
      mapaProductos[key].ingresos += pr * c;
      mapaProductos[key].vecesEnVentas += 1;
    });
  });
...
function renderAnalisisABC(lista, totalGanancia) {
  ...
  if (sorted.length === 0 || totalG <= 0) {
    ...
    ["abcACount", "abcAGanancia", "abcBCount", "abcBGanancia", "abcCCount", "abcCGanancia"].forEach((id) => {
      const e = elID(id);
      if (e) e.textContent = id.includes("Count") ? "0 productos" : "$0 ganancia";
    });
    return;
  }
  if (vacio) vacio.classList.add("hidden");
  ...
  const conClase = sorted.map((p) => {
    const pi = p.ganancia / totalG * 100;
    acum += pi;
...
function exportarAnalisisCSV() {
  ...
  }).sort((a2, b) => b.unidades - a2.unidades);
...
function mostrarAtajos() {
  if (typeof openModal === "function") openModal("atajosModal");
}
...
function actualizarBadgePOS() {
  ...
  if (hoy > 0) {
    badge.textContent = hoy > 99 ? "99+" : hoy;
    badge.style.display = "inline-block";
    badge.title = `${hoy} venta${hoy !== 1 ? "s" : ""} hoy`;
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  btn.id = "btnAtajos";
  btn.title = "Atajos de teclado (Ctrl+Shift+?)";
...
function poblarSelectPedido() {
  ...
  if (!(window.products || []).length) {
    sel.innerHTML = '<option value="">-- Sin productos en inventario --</option>';
    return;
  }
  sel.innerHTML = '<option value="">-- Seleccionar producto (opcional) --</option>' + (window.products || []).map((p) => `<option value="${p.id}">${p.name} - $${Number(p.price).toFixed(2)}</option>`).join("");
}
...
document.addEventListener("submit", function(e) {
  ...
  setTimeout(function() {
    ...
    let target = _editIdSnap ? (window.pedidos || []).find((p) => String(p.id) === String(_editIdSnap)) : (window.pedidos || [])[(window.pedidos || []).length - 1];
    if (target) {
