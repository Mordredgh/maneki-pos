function calcularProducibles(t){if(!Array.isArray(t.mpComponentes)||t.mpComponentes.length===0)return null;let o=1/0;for(const s of t.mpComponentes){const i=(window.products||[]).find(n=>String(n.id)===String(s.id));if(!i)return 0;const r=typeof getStockEfectivo=="function"?getStockEfectivo(i):i.stock||0,a=parseFloat(s.qty)||1;o=Math.min(o,Math.floor(r/a))}return o===1/0?0:o}window.calcularProducibles=calcularProducibles;function abrirBulkPrecioModal(){let t=document.getElementById("bulkPrecioModal");t||(t=document.createElement("div"),t.id="bulkPrecioModal",t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;",t.addEventListener("click",i=>{i.target===t&&(t.style.display="none")}),document.body.appendChild(t));const s=[...new Set((window.products||[]).map(i=>i.category).filter(Boolean))].map(i=>{const r=(window.categories||[]).find(a=>String(a.id)===String(i));return`<option value="${_esc(i)}">${_esc(r?r.emoji?r.emoji+" "+r.name:r.name:i)}</option>`}).join("");t.innerHTML=`
    <div style="background:#fff;border-radius:20px;width:min(540px,95vw);max-height:88vh;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,0.25);">
        ...
        </div>`;return}const u=(document.getElementById("inventorySearch")||{}).value?.trim().toLowerCase()||"",b=(document.getElementById("inventoryTagFilter")||{}).value||"",g=(document.getElementById("inventoryProveedorFilter")||{}).value?.trim().toLowerCase()||"";function P(e){const p=window._normSearch||(f=>String(f||"").toLowerCase()),l=p(u),m=p(g);return e.filter(f=>{const w=!u||p(f.name).includes(l)||p(f.sku||"").includes(l)||p(f.proveedor||"").includes(l)||p(f.notas||"").includes(l)||(f.tags||[]).some(z=>p(z).includes(l)),y=!b||f.tags&&f.tags.includes(b),E=!g||p(f.proveedor||"").includes(m);return w&&y&&E})}const k=P(a.filter(e=>e.tipo==="materia_prima")),I=P(a.filter(e=>e.tipo==="servicio")),x=P(a.filter(e=>e.tipo==="producto_variable")),L=P(a.filter(e=>!e.tipo||e.tipo==="producto"||e.tipo==="producto_interno"||e.tipo==="pack"));function U(e){if(!window._invSortCol)return e;const p=window._invSortCol,l=window._invSortDir;return[...e].sort((m,f)=>{let w,y;return p==="name"?(w=(m.name||"").toLowerCase(),y=(f.name||"").toLowerCase()):p==="sku"?(w=(m.sku||"").toLowerCase(),y=(f.sku||"").toLowerCase()):p==="category"?(w=(m.category||"").toLowerCase(),y=(f.category||"").toLowerCase()):p==="price"?(w=Number(m.price)||0,y=Number(f.price)||0):p==="stock"?(w=Number(m.stock)||0,y=Number(f.stock)||0):p==="margin"&&(w=m.cost&&m.price?(m.price-m.cost)/m.price:-1,y=f.cost&&f.price?(f.price-f.cost)/f.price:-1),w<y?l==="asc"?-1:1:w>y?l==="asc"?1:-1:0})}function J(e,p){const l=String(e.id),m=n.get(l)??(typeof getStockEfectivo=="function"?getStockEfectivo(e):parseInt(e.stock)||0),f=e.imageUrl?`<img src="${e.imageUrl}" style="width:40px;height:40px;object-fit:cover;border-radius:8px;" loading="lazy">`:`<span style="font-size:1.6rem;">${e.image||"\u{1F3ED}"}</span>`;let w;m===0?w='<span class="badge-danger">Agotado</span>':m<=(e.stockMin||5)?w='<span class="badge-warning">Bajo Stock</span>':w='<span class="badge-success">Disponible</span>';const y=(window.categories||[]).find(z=>z.id===e.category),E=y?y.name:e.category||"";return`
        ...
        <tr style="animation:mkSectionIn 0.3s ease both;animation-delay:${p*.03}s" class="hover:bg-indigo-50">
            ...
            <td class="px-4 py-3">
                <div>
                    <span class="font-semibold text-gray-800" style="font-size:.9rem;">${_esc(e.name)}</span>
                    ${e.notas?`<div class="text-xs text-gray-400 truncate" style="max-width:160px;" title="${_esc(e.notas)}">${_esc(e.notas)}</div>`:""}
                    ${e.tags&&e.tags.length?`<div style="display:flex;flex-wrap:wrap;gap:2px;margin-top:2px;">${e.tags.map(f=>`<span style="padding:1px 6px;border-radius:99px;font-size:10px;background:#ede9fe;color:#6d28d9;border:1px solid #ddd6fe;">${_esc(f)}</span>`).join("")}</div>`:""}
                </div>
            </td>
            <td class="px-4 py-3 text-gray-500 text-xs inv-col-hidden-sku">${_esc(e.sku||"\u2014")}</td>
        ...
        </tr>`}function O(e,p){const l=String(e.id),m=e.imageUrl?`<img src="${e.imageUrl}" style="width:40px;height:40px;object-fit:cover;border-radius:8px;" loading="lazy">`:`<span style="font-size:1.6rem;">${e.image||"\u{1F4E6}"}</span>`,f=(window.categories||[]).find(d=>d.id===e.category),w=f?f.name:e.category||"",y=calcularDisponibilidadDesdeMP(e);let E,z;if(y!==null){const d=y.piezas,v=d===0?"#ef4444":d<=3?"#f59e0b":"#10b981",M=d===0?"#fee2e2":d<=3?"#fef3c7":"#d1fae5",_=y.detalle.map(S=>`${S.nombre}: ${S.stock}\xF7${S.qty}=${S.posibles}pzs`).join(" | ");E=`
                <div style="display:flex;flex-direction:column;align-items:flex-start;gap:2px;">
                    <span title="${_esc(_)}"
        ...
        <tr style="animation:mkSectionIn 0.3s ease both;animation-delay:${p*.03}s" class="hover:bg-amber-50">
            ...
            <td class="px-4 py-3">
                <div>
                    ...
                    ${(()=>{const d=calcularProducibles(e);if(d===null)return"";const v=d>=5?"#16a34a":d>=1?"#d97706":"#dc2626",M=d>=5?"#d1fae5":d>=1?"#fef3c7":"#fee2e2",_=d===0?"Sin stock MP":`Producibles: ${d}`;return`<div style="margin-top:3px;"><span style="font-size:9px;font-weight:700;padding:1px 7px;border-radius:99px;background:${M};color:${v};border:1px solid ${v}33;">\u{1F3ED} ${_}</span></div>`})()}
                </div>
            </td>
            <td class="px-4 py-3 text-gray-500 text-xs inv-col-hidden-sku">${_esc(e.sku||"\u2014")}</td>
        ...
        </tr>`}function A(e,p){const l=String(e.id),m=e.imageUrl?`<img src="${e.imageUrl}" style="width:40px;height:40px;object-fit:cover;border-radius:8px;" loading="lazy">`:`<span style="font-size:1.6rem;">${e.image||"\u{1F3AF}"}</span>`,f=(e.tablaPreciosVariable||[]).slice().sort(($,F)=>$.cantidadMin-F.cantidadMin),w=f.length?f.map($=>`<span style="font-size:10px;background:#e0f2fe;color:#0369a1;padding:1px 7px;border-radius:99px;white-space:nowrap;">${$.cantidadMin} pzas = $${Number($.precio).toFixed(2)}</span>`).join(" "):'<span style="font-size:10px;color:#9ca3af;">Sin rangos</span>',y=(e.mpComponentes||[]).length,E=(window.categories||[]).find($=>String($.id)===String(e.category)),z=E?`${E.emoji||""} ${E.name}`:"\u2014",N=f,B=N.length?N[0].precio/(N[0].cantidadMin||1):0,H=B>0?`<div><span class="font-semibold text-gray-800" style="font-size:.95rem;">$${B.toFixed(2)}</span><div style="font-size:10px;color:#9ca3af;">por pieza</div></div>`:'<span style="color:#9ca3af;font-size:.8rem;">\u2014</span>',D=calcularDisponibilidadDesdeMP(e);let d,v;if(D!==null){const $=D.piezas,F=$===0?"#ef4444":$<=3?"#f59e0b":"#10b981",X=$===0?"#fee2e2":$<=3?"#fef3c7":"#d1fae5",T=D.detalle.map(j=>`${j.nombre}: ${j.stock}\xF7${j.qty}=${j.posibles}pzs`).join(" | ");d=`<div style="display:flex;flex-direction:column;align-items:flex-start;gap:2px;">
        ...
        <tr style="animation:mkSectionIn 0.3s ease both;animation-delay:${p*.03}s" class="hover:bg-sky-50">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;
                ...
                <div style="display:flex;gap:4px;">
                    ...
                    ${Array.from({length:Math.min(5,S)},(j,te)=>{let C=h<=3?te+1:h+te-2;return C<1&&(C=null),C>S&&(C=null),C===null?"":`<button onclick="invSectionPage('${e}',${C})" style="min-width:30px;padding:4px 8px;border:1px solid ${C===h?"#C5973B":"#e5e7eb"};border-radius:7px;background:${C===h?"#C5973B":"#fff"};color:${C===h?"#fff":"#374151"};font-weight:${C===h?700:400};font-size:13px;cursor:${C===h?"default":"pointer"};" ${C===h?"disabled":""}>${C}</button>`}).join("")}
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px;">
        ...
        <div style="background:#fff;border:1.5px solid #e5e7eb;border-radius:14px;padding:14px 18px;box-shadow:0 1px 6px #0000000a;">
            <div style="font-size:1.4rem;font-weight:800;color:#7c3aed;">$${ie.toLocaleString("es-MX",{minimumFractionDigits:0,maximumFractionDigits:0})}</div>
        ...
        </div>`:r.innerHTML=ee}function invSectionPage(t,o){const s=`_invPage_${t}`,i=window.products||[],r=t==="mp"?i.filter(g=>g.tipo==="materia_prima"):t==="svc"?i.filter(g=>g.tipo==="servicio"):t==="pv"?i.filter(g=>g.tipo==="producto_variable"):i.filter(g=>!g.tipo||g.tipo==="producto"||g.tipo==="producto_interno"||g.tipo==="pack"),a=(document.getElementById("inventorySearch")||{}).value?.trim().toLowerCase()||"",n=(document.getElementById("inventoryTagFilter")||{}).value||"",c=(document.getElementById("inventoryProveedorFilter")||{}).value?.trim().toLowerCase()||"",u=r.filter(g=>{const P=!a||g.name.toLowerCase().includes(a)||(g.sku||"").toLowerCase().includes(a)||(g.proveedor||"").toLowerCase().includes(a)||(g.notas||"").toLowerCase().includes(a)||(g.tags||[]).some(x=>x.toLowerCase().includes(a)),k=!n||g.tags&&g.tags.includes(n),I=!c||(g.proveedor||"").toLowerCase().includes(c);return P&&k&&I}),b=Math.max(1,Math.ceil(u.length/(window._invPageSize||10)));window[s]=Math.max(1,Math.min(o,b)),renderInventoryTable()}window.invSectionPage=invSectionPage;function _renderInventoryPagination(t,o,s,i,r){let a=document.getElementById("inventoryPaginationBar");if(!a){const b=document.getElementById("inventoryTable")?.closest('table, .overflow-x-auto, [class*="overflow"]');if(!b)return;a=document.createElement("div"),a.id="inventoryPaginationBar",b.insertAdjacentElement("afterend",a)}if(o<=1&&s<=r){a.innerHTML="";return}const n=Math.min(i+r,s),c=`Mostrando <b>${i+1}\u2013${n}</b> de <b>${s}</b> productos`;function u(){const b=[],g=(P,k)=>{for(let I=P;I<=k;I++)b.push(I)};return o<=7?g(1,o):(b.push(1),t>4&&b.push("..."),g(Math.max(2,t-2),Math.min(o-1,t+2)),t<o-3&&b.push("..."),b.push(o)),b.map(P=>{if(P==="...")return'<span style="padding:0 4px;color:#9ca3af;">\u2026</span>';const k=P===t;return`<button onclick="invGoToPage(${P})"
...
`});const J=new Blob([U],{type:"text/csv;charset=utf-8;"}),G=URL.createObjectURL(J),O=document.createElement("a");O.href=G,O.download=`movimientos-${a}.csv`,O.click(),URL.revokeObjectURL(G)},o.parentNode.insertBefore(k,o)),!r.length){o.innerHTML='<p class="text-gray-400 text-sm text-center py-4">Sin movimientos registrados</p>';return}const I={entrada:"\u{1F7E2}",salida:"\u{1F534}",ajuste:"\u{1F7E1}",creacion:"\u{1F535}",venta:"\u{1F7E0}",merma:"\u{1F7E4}"};o.innerHTML=r.slice(0,200).map(x=>{const L=new Date(x.fecha).toLocaleString("es-MX",{dateStyle:"short",timeStyle:"short"}),U=x.cantidad>=0?`+${x.cantidad}`:`${x.cantidad}`;return`<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:13px;">
        ...
        </div>`}).join("")}window.renderStockMovements=renderStockMovements;function duplicarProducto(t){const o=(window.products||[]).find(i=>String(i.id)===String(t));if(!o){manekiToastExport("Producto no encontrado","err");return}const s=JSON.parse(JSON.stringify(o));s.id=_genId(),s.name="Copia de "+o.name,s.sku=(o.sku||"")+"-C",s.stock=0,s.historialPrecios=[],s.historialCostos=[],window.products.unshift(s),saveProducts(),renderInventoryTable(),manekiToastExport(`\u{1F4CB} "${s.name}" creado \u2014 ed\xEDtalo para ajustar stock y SKU`,"ok")}window.duplicarProducto=duplicarProducto;function abrirReporteRentabilidad(){const t=(window.products||[]).filter(n=>!n.tipo||n.tipo==="producto"||n.tipo==="producto_interno"),o=t.map(n=>{const c=n.price>0&&n.cost>0?(n.price-n.cost)/n.price*100:null;return{...n,_margen:c}}).sort((n,c)=>(c._margen??-1/0)-(n._margen??-1/0)),s=o.map((n,c)=>{const u=n._margen!==null?n._margen.toFixed(1)+"%":"\u2014",b=n.price>0&&n.cost>0?"$"+(n.price-n.cost).toFixed(2):"\u2014",g=n._margen===null?"#9ca3af":n._margen>=50?"#16a34a":n._margen>=30?"#d97706":"#dc2626";return`<tr style="border-bottom:1px solid #f3f4f6;">
