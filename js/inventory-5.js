function _levenshtein(t,o){const i=t.length,r=o.length,a=Array.from({length:i+1},(s,n)=>Array.from({length:r+1},(l,p)=>p===0?n:0));for(let s=1;s<=r;s++)a[0][s]=s;for(let s=1;s<=i;s++)for(let n=1;n<=r;n++)a[s][n]=t[s-1]===o[n-1]?a[s-1][n-1]:1+Math.min(a[s-1][n],a[s][n-1],a[s-1][n-1]);return a[i][r]}function _fuzzyMatch(t,o,i=2){return t=t.toLowerCase().trim(),o=o.toLowerCase(),!t||o.includes(t)?!0:o.split(/[\s,.-]+/).some(a=>{const s=a.substring(0,t.length+2);return s.length>=t.length-1&&_levenshtein(t,s)<=i})}window._fuzzyMatch=_fuzzyMatch;function calcularProducibles(t){if(!Array.isArray(t.mpComponentes)||t.mpComponentes.length===0)return null;let o=1/0;for(const i of t.mpComponentes){const r=(window.products||[]).find(n=>String(n.id)===String(i.id));if(!r)return 0;const a=typeof getStockEfectivo=="function"?getStockEfectivo(r):r.stock||0,s=parseFloat(i.qty)||1;o=Math.min(o,Math.floor(a/s))}return o===1/0?0:o}window.calcularProducibles=calcularProducibles;function abrirBulkPrecioModal(){let t=document.getElementById("bulkPrecioModal");t||(t=document.createElement("div"),t.id="bulkPrecioModal",t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;",t.addEventListener("click",r=>{r.target===t&&(t.style.display="none")}),document.body.appendChild(t));const i=[...new Set((window.products||[]).map(r=>r.category).filter(Boolean))].map(r=>{const a=(window.categories||[]).find(s=>String(s.id)===String(r));return`<option value="${_esc(r)}">${_esc(a?a.emoji?a.emoji+" "+a.name:a.name:r)}</option>`}).join("");t.innerHTML=`
    <div style="background:#fff;border-radius:20px;width:min(540px,95vw);max-height:88vh;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,0.25);">
        ...
        </div>`;return}const p=(document.getElementById("inventorySearch")||{}).value?.trim().toLowerCase()||"",m=(document.getElementById("inventoryTagFilter")||{}).value||"",f=(document.getElementById("inventoryProveedorFilter")||{}).value?.trim().toLowerCase()||"";function M(e){const u=window._normSearch||(g=>String(g||"").toLowerCase()),d=u(p),y=u(f),b=g=>!m||g.tags&&g.tags.includes(m),h=g=>!f||u(g.proveedor||"").includes(y);if(!p)return e.filter(g=>b(g)&&h(g));const v=e.filter(g=>(u(g.name).includes(d)||u(g.sku||"").includes(d)||u(g.proveedor||"").includes(d)||u(g.notas||"").includes(d)||(g.tags||[]).some(F=>u(F).includes(d)))&&b(g)&&h(g));return v.length>0?v:e.filter(g=>(_fuzzyMatch(d,g.name||"")||_fuzzyMatch(d,g.sku||"")||_fuzzyMatch(d,g.proveedor||""))&&b(g)&&h(g))}const $=M(s.filter(e=>e.tipo==="materia_prima")),L=M(s.filter(e=>e.tipo==="servicio")),x=M(s.filter(e=>e.tipo==="producto_variable")),j=M(s.filter(e=>!e.tipo||e.tipo==="producto"||e.tipo==="producto_interno"||e.tipo==="pack"));function R(e){if(!window._invSortCol)return e;const u=window._invSortCol,d=window._invSortDir;return[...e].sort((y,b)=>{let h,v;return u==="name"?(h=(y.name||"").toLowerCase(),v=(b.name||"").toLowerCase()):u==="sku"?(h=(y.sku||"").toLowerCase(),v=(b.sku||"").toLowerCase()):u==="category"?(h=(y.category||"").toLowerCase(),v=(b.category||"").toLowerCase()):u==="price"?(h=Number(y.price)||0,v=Number(b.price)||0):u==="stock"?(h=Number(y.stock)||0,v=Number(b.stock)||0):u==="margin"&&(h=y.cost&&y.price?(y.price-y.cost)/y.price:-1,v=b.cost&&b.price?(b.price-b.cost)/b.price:-1),h<v?d==="asc"?-1:1:h>v?d==="asc"?1:-1:0})}function J(e,u){const d=String(e.id),y=n.get(d)??(typeof getStockEfectivo=="function"?getStockEfectivo(e):parseInt(e.stock)||0),b=e.imageUrl?`<img src="${e.imageUrl}" style="width:40px;height:40px;object-fit:cover;border-radius:8px;" loading="lazy">`:`<span style="font-size:1.6rem;">${e.image||"\u{1F3ED}"}</span>`;let h;y===0?h='<span class="badge-danger">Agotado</span>':y<=(e.stockMin||5)?h='<span class="badge-warning">Bajo Stock</span>':h='<span class="badge-success">Disponible</span>';const v=(window.categories||[]).find(z=>z.id===e.category),g=v?v.name:e.category||"";return`
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px;">
        ...
        </div>`:a.innerHTML=te}function invSectionPage(t,o){const i=`_invPage_${t}`,r=window.products||[],a=t==="mp"?r.filter(f=>f.tipo==="materia_prima"):t==="svc"?r.filter(f=>f.tipo==="servicio"):t==="pv"?r.filter(f=>f.tipo==="producto_variable"):r.filter(f=>!f.tipo||f.tipo==="producto"||f.tipo==="producto_interno"||f.tipo==="pack"),s=(document.getElementById("inventorySearch")||{}).value?.trim().toLowerCase()||"",n=(document.getElementById("inventoryTagFilter")||{}).value||"",l=(document.getElementById("inventoryProveedorFilter")||{}).value?.trim().toLowerCase()||"",p=a.filter(f=>{const M=!s||f.name.toLowerCase().includes(s)||(f.sku||"").toLowerCase().includes(s)||(f.proveedor||"").toLowerCase().includes(s)||(f.notas||"").toLowerCase().includes(s)||(f.tags||[]).some(x=>x.toLowerCase().includes(s)),$=!n||f.tags&&f.tags.includes(n),L=!l||(f.proveedor||"").toLowerCase().includes(l);return M&&$&&L}),m=Math.max(1,Math.ceil(p.length/(window._invPageSize||10)));window[i]=Math.max(1,Math.min(o,m)),renderInventoryTable()}window.invSectionPage=invSectionPage;function _renderInventoryPagination(t,o,i,r,a){let s=document.getElementById("inventoryPaginationBar");if(!s){const m=document.getElementById("inventoryTable")?.closest('table, .overflow-x-auto, [class*="overflow"]');if(!m)return;s=document.createElement("div"),s.id="inventoryPaginationBar",m.insertAdjacentElement("afterend",s)}if(o<=1&&i<=a){s.innerHTML="";return}const n=Math.min(r+a,i),l=`Mostrando <b>${r+1}\u2013${n}</b> de <b>${i}</b> productos`;function p(){const m=[],f=(M,$)=>{for(let L=M;L<=$;L++)m.push(L)};return o<=7?f(1,o):(m.push(1),t>4&&m.push("..."),f(Math.max(2,t-2),Math.min(o-1,t+2)),t<o-3&&m.push("..."),m.push(o)),m.map(M=>{if(M==="...")return'<span style="padding:0 4px;color:#9ca3af;">\u2026</span>';const $=M===t;return`<button onclick="invGoToPage(${M})"
...
`});const J=new Blob([R],{type:"text/csv;charset=utf-8;"}),Q=URL.createObjectURL(J),q=document.createElement("a");q.href=Q,q.download=`movimientos-${s}.csv`,q.click(),URL.revokeObjectURL(Q)},o.parentNode.insertBefore($,o)),!a.length){o.innerHTML='<p class="text-gray-400 text-sm text-center py-4">Sin movimientos registrados</p>';return}const L={entrada:"\u{1F7E2}",salida:"\u{1F534}",ajuste:"\u{1F7E1}",creacion:"\u{1F535}",venta:"\u{1F7E0}",merma:"\u{1F7E4}"};o.innerHTML=a.slice(0,200).map(x=>{const j=new Date(x.fecha).toLocaleString("es-MX",{dateStyle:"short",timeStyle:"short"}),R=x.cantidad>=0?`+${x.cantidad}`:`${x.cantidad}`;return`<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:13px;">
        ...
        </div>`}).join("")}window.renderStockMovements=renderStockMovements;function duplicarProducto(t){const o=(window.products||[]).find(r=>String(r.id)===String(t));if(!o){manekiToastExport("Producto no encontrado","err");return}const i=JSON.parse(JSON.stringify(o));i.id=_genId(),i.name="Copia de "+o.name,i.sku=(o.sku||"")+"-C",i.stock=0,i.historialPrecios=[],i.historialCostos=[],window.products.unshift(i),saveProducts(),renderInventoryTable(),manekiToastExport(`\u{1F4CB} "${i.name}" creado \u2014 ed\xEDtalo para ajustar stock y SKU`,"ok")}window.duplicarProducto=duplicarProducto;function abrirReporteRentabilidad(){const t=(window.products||[]).filter(n=>!n.tipo||n.tipo==="producto"||n.tipo==="producto_interno"),o=t.map(n=>{const l=n.price>0&&n.cost>0?(n.price-n.cost)/n.price*100:null;return{...n,_margen:l}}).sort((n,l)=>(l._margen??-1/0)-(n._margen??-1/0)),i=o.map((n,l)=>{const p=n._margen!==null?n._margen.toFixed(1)+"%":"\u2014",m=n.price>0&&n.cost>0?"$"+(n.price-n.cost).toFixed(2):"\u2014",f=n._margen===null?"#9ca3af":n._margen>=50?"#16a34a":n._margen>=30?"#d97706":"#dc2626";return`<tr style="border-bottom:1px solid #f3f4f6;">
        ...
        </tr>`}).join(""),r=o.filter(n=>n._margen!==null).reduce((n,l,p,m)=>n+l._margen/m.length,0),a=o[0];let s=document.getElementById("_mkRentabilidadModal");s||(s=document.createElement("div"),s.id="_mkRentabilidadModal",s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;",s.addEventListener("click",n=>{n.target===s&&(s.style.display="none")}),document.body.appendChild(s)),s.innerHTML=`
