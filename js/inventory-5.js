    ...
    <div style="background:#fff;border-radius:20px;width:min(540px,95vw);max-height:88vh;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,0.25);">
        ...
        <tr style="animation:mkSectionIn 0.3s ease both;animation-delay:${u*.03}s" class="hover:bg-indigo-50">
            ...
            <td class="px-2 py-3">
                ...
                </span>`).join(""):'<span class="text-xs text-gray-400">Sin variantes</span>',C=Number(e.cost)||0,N=Number(e.price)||0,D=C&&N?(()=>{const c=(N-C)/N*100,w=c>=40?"#10b981":c>=20?"#f59e0b":"#ef4444";return`<div style="min-width:56px;">
        ...
        <tr style="animation:mkSectionIn 0.3s ease both;animation-delay:${u*.03}s" class="hover:bg-sky-50">
        ...
        </tr>`}function U({id:e,title:u,titleColor:d,titleBg:y,btnLabel:b,btnOnclick:h,btnColor:v,extraBtnHTML:g,products:z,renderFila:F,headers:C,emptyMsg:N}){const D=(document.getElementById("inventorySearch")?.value?.trim()||"").length>0;if(z.length===0&&D)return"";const c=R(z),w=`_invPage_${e}`,M=window._invPageSize||10;window[w]=window[w]||1;const _=c.length,S=Math.max(1,Math.ceil(_/M));window[w]>S&&(window[w]=1);const k=window[w],K=(k-1)*M,X=c.slice(K,K+M),E=X.length===0?`<tr><td colspan="${C.length}" style="padding:32px;text-align:center;color:#9ca3af;font-size:.85rem;">${N}</td></tr>`:X.map((T,I)=>F(T,I)).join(""),H=C.map(T=>{const I=T.colId==="sku"?" inv-col-hidden-sku":T.colId==="proveedor"?" inv-col-hidden-prov":"",G=T.align==="right"?" text-right":" text-left";return T.sortKey?`<th class="px-4 py-3${G} text-xs font-semibold text-gray-500 uppercase tracking-wide sortable-th cursor-pointer select-none${I}" onclick="sortInventory('${T.sortKey}')" style="white-space:nowrap;">${T.label} \u2195</th>`:`<th class="px-4 py-3${G} text-xs font-semibold text-gray-500 uppercase tracking-wide${I}" style="white-space:nowrap;">${T.label}</th>`}).join("");let W="";if(S>1||_>M){const T=Math.min(K+M,_);W=`
            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;
                ...
                <div style="display:flex;gap:4px;">
                    ...
                    ${Array.from({length:Math.min(5,S)},(I,G)=>{let B=k<=3?G+1:k+G-2;return B<1&&(B=null),B>S&&(B=null),B===null?"":`<button onclick="invSectionPage('${e}',${B})" style="min-width:30px;padding:4px 8px;border:1px solid ${B===k?"#C5973B":"#e5e7eb"};border-radius:7px;background:${B===k?"#C5973B":"#fff"};color:${B===k?"#fff":"#374151"};font-weight:${B===k?700:400};font-size:13px;cursor:${B===k?"default":"pointer"};" ${B===k?"disabled":""}>${B}</button>`}).join("")}
