...
function bulkPrecioAplicar() {
  const afectados = _bulkPrecioGetAfectados();
  if (!afectados.length) {
    manekiToastExport("Sin productos que actualizar", "warn");
    return;
  }
  const pct = parseFloat(document.getElementById("bulkPrecioNum")?.value) || 0;
...
function renderInventoryTable() {
  ...
  function applyFilters(list) {
    ...
    return list.filter((p) => {
      const nombreMatch = !q || _ns(p.name).includes(qN) || _ns(p.sku || "").includes(qN) || _ns(p.proveedor || "").includes(qN) || _ns(p.notas || "").includes(qN) || (p.tags || []).some((t) => _ns(t).includes(qN));
  ...
  function renderFilaPT(product, ri) {
    ...
    if (disp !== null) {
      ...
      badgeCell = piezas === 0 ? '<span class="badge-danger">Sin stock MP</span>' : piezas <= 3 ? '<span class="badge-warning">MP bajo</span>' : '<span class="badge-success">Disponible</span>';
    } else {
    ...
    const varsHTML = product.variants && product.variants.length > 0 ? product.variants.map((v) => `
                ...
                </span>`).join("") : '<span class="text-xs text-gray-400">Sin variantes</span>';
  ...
  function renderFilaVariable(product, ri) {
    ...
    const margenHTML = _pvPrecioMin > 0 ? `<div style="min-width:48px;">
  ...
  const margenProm = ptConPrecio.length ? ptConPrecio.reduce((s, p) => {
    const pr = Number(p.price) || 0, co = Number(p.cost) || 0;
    return s + (pr > 0 ? (pr - co) / pr * 100 : 0);
  }, 0) / ptConPrecio.length : 0;
...
function invSectionPage(sectionId, page) {
  ...
  const allProducts = window.products || [];
  const list = sectionId === "mp" ? allProducts.filter((p) => p.tipo === "materia_prima") : sectionId === "svc" ? allProducts.filter((p) => p.tipo === "servicio") : sectionId === "pv" ? allProducts.filter((p) => p.tipo === "producto_variable") : allProducts.filter((p) => !p.tipo || p.tipo === "producto" || p.tipo === "producto_interno" || p.tipo === "pack");
  const q = (document.getElementById("inventorySearch") || {}).value?.trim().toLowerCase() || "";
  const tagQ = (document.getElementById("inventoryTagFilter") || {}).value || "";
  const provQ = (document.getElementById("inventoryProveedorFilter") || {}).value?.trim().toLowerCase() || "";
  const filteredList = list.filter((p) => {
    const nombreMatch = !q || p.name.toLowerCase().includes(q) || (p.sku || "").toLowerCase().includes(q) || (p.proveedor || "").toLowerCase().includes(q) || (p.notas || "").toLowerCase().includes(q) || (p.tags || []).some((t) => t.toLowerCase().includes(q));
    const tagMatch = !tagQ || p.tags && p.tags.includes(tagQ);
...
function renderMovimientos() {
  ...
  const hoy = _fechaHoy ? _fechaHoy() : (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  const movsHoy = (window.stockMovements || []).filter((m) => {
    try {
      const f = new Date(m.fecha);
      return f.getFullYear() + "-" + ("0" + (f.getMonth() + 1)).slice(-2) + "-" + ("0" + f.getDate()).slice(-2) === hoy;
    } catch (e) {
      return false;
    }
  });
...
async function invBulkEliminar() {
  ...
  const _pedAfectados = (window.pedidos || []).filter(
    (ped) => !["cancelado", "finalizado"].includes(ped.status || "") && (ped.productosInventario || []).some((item) => ids.includes(String(item.id)))
  );
...
${(window.categories || []).map((c) => `${c.id}: ${c.emoji || ""} ${c.name}`).join("\n")}`);
  ...
  (window.products || []).forEach((p) => {
    if (ids.includes(String(p.id))) p.category = cat.id;
  });
  saveProducts();
  renderInventoryTable();
