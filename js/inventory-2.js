...
function toggleTagPt(tag) {
  ...
  if (i > -1) window._ptTagsActuales.splice(i, 1);
  else window._ptTagsActuales.push(tag);
  renderTagsPt();
}
...
function toggleTagPv(tag) {
  ...
  if (i > -1) window._pvTagsActuales.splice(i, 1);
  else window._pvTagsActuales.push(tag);
  renderTagsPv();
}
...
function renderVariantsListPt() {
  ...
  if (!window._ptVariants || !window._ptVariants.length) {
    el.innerHTML = '<p style="font-size:.8rem;color:#9ca3af;">Sin variantes agregadas</p>';
    return;
  }
...
function abrirSelectorMpPt() {
  ...
  if (box.style.display === "block") {
    filtrarMpSelector();
    document.getElementById("ptMpSearch")?.focus();
  }
}
...
function filtrarMpSelector() {
  ...
  if (!lista.length) {
    res.innerHTML = '<p style="font-size:.8rem;color:#9ca3af;padding:8px;">No hay materias primas ni servicios registrados</p>';
    return;
  }
...
function seleccionarMpPt(id) {
  ...
  if (Array.isArray(mp.variants) && mp.variants.length > 0) {
    ...
    mp.variants.forEach((v) => {
      ...
      const yaExiste = window._ptVariants.some(
        (pv) => (pv.type || pv.tipo) === varType && (pv.value || pv.valor) === varValue
      );
      if (!yaExiste) {
...
function ptActualizarPrecioSugerido() {
  ptMostrarMargenInfo();
}
...
async function guardarProductoTerminado() {
  ...
  const _nombreDupPt = (window.products || []).find(
  ...
  try {
    ...
    if (window.modoEdicion && window.edicionProductoId !== null) {
      ...
      window.products[idx] = Object.assign({}, window.products[idx], {
        name: nombre,
        category: catId,
        tipo: publicarTienda ? "producto" : "producto_interno",
        cost: costoFinal,
        price: precio,
        stockMin,
        tags,
      ...
      if (stNew !== stOld) registrarMovimiento({
...
function packFiltrarPT() {
  ...
  const pts = (window.products || []).filter(
    (p) => (p.tipo === "producto" || p.tipo === "producto_interno") && !yaAgregados.includes(String(p.id)) && (!q || p.name.toLowerCase().includes(q))
  );
...
function packFiltrarMP() {
  ...
  const mps = (window.products || []).filter(
    (p) => (p.tipo === "materia_prima" || p.tipo === "servicio") && !yaAgregados.includes(String(p.id)) && (!q || p.name.toLowerCase().includes(q))
  );
...
function packActualizarQtyMP(mpId, valor) {
  const mp = (window._packMpDirectos || []).find((m) => String(m.id) === String(mpId));
  if (mp) mp.qty = Math.max(1e-3, parseFloat(valor) || 1);
  packRenderMpDirectos();
}
...
async function guardarPack() {
  ...
  const nombreDup = (window.products || []).find(
  ...
  try {
    ...
    if (window._packModoEdicion && window._packEdicionId) {
      ...
      if (idx === -1) {
        manekiToastExport("Error: pack no encontrado", "err");
        return;
      }
      const anterior = window.products[idx];
      const historial = anterior.historialPrecios || [];
      if (anterior.price !== precio || anterior.cost !== costo) {
        historial.push({ fecha: (/* @__PURE__ */ new Date()).toISOString(), precioAntes: anterior.price, costoAntes: anterior.cost, precioNuevo: precio, costoNuevo: costo });
      }
...
function pvFiltrarMP(q) {
  ...
  if (!box) return;
  const mps = (window.products || []).filter(
    (p) => p.tipo === "materia_prima" || p.tipo === "servicio"
  ...
  box.style.display = "block";
  box.innerHTML = mps.slice(0, 8).map(
    (p) => `<div onclick="pvSeleccionarMP('${p.id}')"
...
async function guardarProductoVariable(e) {
  ...
  if (editId) {
    ...
    window.products[idx] = Object.assign({}, window.products[idx], {
      name: nombre,
      tipo: "producto_variable",
      sku: finalSku,
      rendimientoPorHoja: rendimiento,
      mpComponentes: mpComps,
      tablaPreciosVariable: tabla,
      cost: costoHoja,
...
function pvGetPrecio(product, cantidad) {
  ...
  return rangoElegido.precio / min;
}
