...
</td></tr>`,updateClientStats();return}if(s.length===0){a.innerHTML='<tr><td colspan="7"><p style="text-align:center;padding:32px;color:#9ca3af;font-size:.9rem">Sin clientes con ese filtro</p></td></tr>',updateClientStats();return}const l=a.closest("table")?.querySelector("thead tr");if(l){const i=[{key:"name",label:"Cliente"},{key:null,label:"Contacto"},{key:null,label:"Email"},{key:"totalPurchases",label:"Total Compras"},{key:"lastPurchase",label:"\xDAltima Compra"},{key:null,label:"Tipo"},{key:null,label:"Acciones"}];l.innerHTML=i.map(d=>{const f=d.key==="totalPurchases"?"text-right":"text-left";return d.key?`<th class="px-6 py-3 ${f} text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:text-amber-600 select-none" onclick="sortClientes('${_escAttr(d.key)}')">${d.label} ${_sortArrow(d.key)}</th>`:`<th class="px-6 py-3 ${f} text-xs font-semibold text-gray-500 uppercase tracking-wider">${d.label}</th>`}).join("")}function e(i){const d=(i||"").trim().toLowerCase().charCodeAt(0);return d>=97&&d<=101?"mk-avatar-gold":d>=102&&d<=108?"mk-avatar-lila":d>=109&&d<=114?"mk-avatar-peach":"mk-avatar-green"}function c(i){const d=(i||"").trim().split(" ");return((d[0]||"")[0]||"")+(d[1]?d[1][0]:"")}const m=_clientesSortCol,r=_clientesSortDir==="asc"?1:-1,p=[...s].sort((i,d)=>{let f,g;return m==="totalPurchases"?(f=Number(i.totalPurchases||0),g=Number(d.totalPurchases||0),r*(f-g)):m==="lastPurchase"?(f=i.lastPurchase||"",g=d.lastPurchase||"",r*f.localeCompare(g)):(f=(i.name||"").toLowerCase(),g=(d.name||"").toLowerCase(),r*f.localeCompare(g))}),u={};(window.clientes||clients||[]).forEach(i=>{const d=i.id||i.nombre||i.name||"";u[d]=_calcClienteStats(i.id||i.nombre||i.name||"")}),a.innerHTML=p.map((i,d)=>{const f=i.isVIP||i.type==="vip",g=c(i.name||"?").toUpperCase()||(i.name||"?").trim().charAt(0).toUpperCase(),y=e(i.name),h=(window.notas||[]).filter(x=>x.cliente&&x.cliente.toLowerCase()===(i.name||"").toLowerCase()).sort((x,C)=>(C.fechaCreacion||C.fecha||"").localeCompare(x.fechaCreacion||x.fecha||"")),$=h.length>0?`<div class="text-xs text-gray-400 mt-0.5 truncate max-w-[180px]" title="${_escAttr(h[0].texto)}">\u{1F4DD} ${_esc((h[0].texto||"").substring(0,40))}${(h[0].texto||"").length>40?"\u2026":""}</div>`:"",k=i.notas?`<div style="font-size:.72rem;color:#6b7280;margin-top:2px" title="${_escAttr(i.notas)}">\u{1F4DD} ${_esc(i.notas.substring(0,60))}${i.notas.length>60?"\u2026":""}</div>`:"",w=i.id||i.nombre||i.name||"",b=u[w]||_calcClienteStats(w),E=`<div style="font-size:.7rem;color:#6b7280;margin-top:4px;display:flex;gap:6px;flex-wrap:wrap">
...
<td class="px-6 py-4 text-gray-600 text-sm">${i.email?_esc(i.email):"\u2014"}</td>
                    <td class="px-6 py-4 text-right text-gray-800 font-semibold">${fmtMoney(i.totalPurchases||0)}</td>
    ...
    <div class="flex items-center gap-3">
        ...
        <button onclick="deleteClient('${_escAttr(i.id)}')" class="text-red-500 hover:text-red-700" title="Eliminar">
            ...
            `}).join(""),updateClientStats()}function updateClientStats(){document.getElementById("totalClients").textContent=clients.length,document.getElementById("vipClients").textContent=clients.filter(o=>o.isVIP||o.type==="vip").length;const n=clients.reduce((o,t)=>o+(Number(t.totalPurchases)||0),0);document.getElementById("totalPurchases").textContent=fmtMoney(n)}let selectedClientType="regular";function selectClientType(n){selectedClientType=n,document.getElementById("clientType").value=n;const o=document.getElementById("btnClientRegular"),t=document.getElementById("btnClientVip");n==="vip"?(t.style.borderColor="#C5973B",t.style.background="#FFF9F0",t.style.color="#C5973B",o.style.borderColor="#E5E7EB",o.style.background="white",o.style.color="#6B7280"):(o.style.borderColor="#C5973B",o.style.background="#FFF9F0",o.style.color="#C5973B",t.style.borderColor="#E5E7EB",t.style.background="white",t.style.color="#6B7280")}function openAddClientModal(){document.getElementById("clientModalTitle").textContent="Nuevo Cliente",document.getElementById("clientSubmitBtn").innerHTML='<i class="fas fa-save mr-2"></i>Guardar Cliente',document.getElementById("editClientId").value="",document.getElementById("addClientForm").reset(),selectClientType("regular"),openModal("addClientModal")}function closeAddClientModal(){closeModal("addClientModal"),document.getElementById("addClientForm").reset()}document.getElementById("addClientForm").addEventListener("submit",function(n){n.preventDefault();const o=document.getElementById("editClientId").value,t=document.getElementById("clientName").value.trim(),a=document.getElementById("clientPhone").value.trim(),s=document.getElementById("clientFacebook").value.trim(),l=document.getElementById("clientEmail").value.trim();if(!_validEmail(l)){manekiToastExport("Email inv\xE1lido","warn");return}const e=document.getElementById("clientType").value||"regular",c=(document.getElementById("clientNotas")?.value||"").trim();if(o){const r=clients.find(p=>String(p.id)===String(o));r&&(r.name=t,r.phone=a,r.facebook=s,r.email=l,r.type=e,r.isVIP=e==="vip",r.notas=c)}else{let p=function(f){return String(f||"").toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g," ")};var m=p;const r={id:mkId(),name:t,phone:a,facebook:s,email:l,type:e,isVIP:e==="vip",notas:c,totalPurchases:0,lastPurchase:null},u=p(t),i=String(a||"").replace(/\D/g,"").slice(-8),d=clients.find(f=>{const g=p(f.name||"")===u&&u!=="",y=i.length>=6&&String(f.phone||"").replace(/\D/g,"").slice(-8)===i;return g||y});d&&manekiToastExport(`\u26A0\uFE0F Ya existe un cliente similar: "${d.name}". Verifica si es el mismo.`,"warn"),clients.push(r)}saveClients(),closeAddClientModal(),renderClientsTable(),updateDashboard()});function editClient(n){const o=clients.find(t=>String(t.id)===String(n));o&&(document.getElementById("clientModalTitle").textContent="Editar Cliente",document.getElementById("clientSubmitBtn").innerHTML='<i class="fas fa-save mr-2"></i>Actualizar Cliente',document.getElementById("editClientId").value=n,document.getElementById("clientName").value=o.name||"",document.getElementById("clientPhone").value=o.phone||"",document.getElementById("clientFacebook").value=o.facebook||"",document.getElementById("clientEmail").value=o.email||"",document.getElementById("clientNotas")&&(document.getElementById("clientNotas").value=o.notas||""),selectClientType(o.type||"regular"),openModal("addClientModal"))}function deleteClient(n){const o=clients.find(c=>String(c.id)===String(n)),t=o?o.name:"este cliente",s=(window.pedidos||[]).filter(c=>String(c.clienteId||"")===String(n)||String(c.cliente||"").toLowerCase()===String(t).toLowerCase()).filter(c=>c.status!=="finalizado"&&c.status!=="cancelado"&&c.status!=="entregado"),l=s.length>0?`Este cliente tiene ${s.length} pedido(s) activo(s). \xBFDeseas eliminarlo de todas formas? Los pedidos quedar\xE1n sin cliente asignado.
...
${!e.phone&&!e.facebook?"\u2014":""}
    ...
    <div class="flex items-center gap-3">
        ...
        `,document.body.appendChild(s)}const n=(window.clients||[]).slice().sort((s,l)=>(s.name||"").localeCompare(l.name||"")),o=n.map(s=>`<option value="${_esc(String(s.id))}">${_esc(s.name)}</option>`).join(""),t=document.getElementById("fusionClientePrincipal"),a=document.getElementById("fusionClienteAFusionar");t&&(t.innerHTML=o),a&&(a.innerHTML=o),a&&n.length>1&&(a.selectedIndex=1),openModal("fusionarClientesModal")}function _ejecutarFusion(){const n=document.getElementById("fusionClientePrincipal"),o=document.getElementById("fusionClienteAFusionar");if(!n||!o)return;const t=n.value,a=o.value;if(t===a){typeof window.manekiToastExport=="function"&&window.manekiToastExport("Selecciona dos clientes diferentes","warn");return}const s=window.clients||[],l=s.find(r=>String(r.id)===String(t)),e=s.find(r=>String(r.id)===String(a));if(!l||!e)return;const c=window.showConfirm;(typeof c=="function"?c(`\xBFFusionar "${_esc(e.name)}" \u2192 "${_esc(l.name)}"?

Sus pedidos e historial se mover\xE1n al cliente principal. Esta acci\xF3n no se puede deshacer.`,"\u{1F500} Fusionar clientes"):Promise.resolve(window.confirm(`\xBFFusionar "${e.name}" en "${l.name}"?`))).then(r=>{if(!r)return;const p=(e.name||"").toLowerCase().trim();(window.pedidos||[]).forEach(u=>{((u.cliente||"").toLowerCase().trim()===p||String(u.clienteId)===String(a))&&(u.cliente=l.name,u.clienteId=l.id)}),(window.pedidosFinalizados||[]).forEach(u=>{(u.cliente||"").toLowerCase().trim()===p&&(u.cliente=l.name)}),(window.salesHistory||[]).forEach(u=>{(u.customer||"").toLowerCase().trim()===p&&(u.customer=l.name)}),!l.phone&&e.phone&&(l.phone=e.phone),!l.email&&e.email&&(l.email=e.email),!l.notas&&e.notas&&(l.notas=e.notas),window.clients=s.filter(u=>String(u.id)!==String(a)),typeof window.saveClients=="function"&&window.saveClients(),typeof window.savePedidos=="function"&&window.savePedidos(),typeof window.saveSalesHistory=="function"&&window.saveSalesHistory(),typeof window.deleteClientFromDB=="function"&&window.deleteClientFromDB(String(a)),closeModal("fusionarClientesModal"),typeof window.renderClientsTable=="function"&&window.renderClientsTable(),typeof window.manekiToastExport=="function"&&window.manekiToastExport(`\u2705 "${e.name}" fusionado con "${l.name}"`,"ok")})}window.abrirFusionarClientes=abrirFusionarClientes,window._ejecutarFusion=_ejecutarFusion,(function(){const o=()=>{if(document.getElementById("_mkBtnFusionarClientes"))return;const t=document.getElementById("addClientBtn");if(!t){setTimeout(o,800);return}const a=document.createElement("button");a.id="_mkBtnFusionarClientes",a.type="button",a.title="Fusionar clientes duplicados",a.textContent="\u{1F500} Fusionar duplicados",a.style.cssText="padding:6px 14px;border-radius:10px;font-size:.78rem;font-weight:600;cursor:pointer;border:1px solid #d1d5db;background:#fff;color:#374151;transition:border-color .15s;margin-left:8px;",a.addEventListener("mouseenter",()=>{a.style.borderColor="#C5973B",a.style.color="#C5973B"}),a.addEventListener("mouseleave",()=>{a.style.borderColor="#d1d5db",a.style.color="#374151"}),a.addEventListener("click",()=>abrirFusionarClientes()),t.insertAdjacentElement("afterend",a)};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",o):o()})();
