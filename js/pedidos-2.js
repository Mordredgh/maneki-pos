...
function setPedidoStatus(status) {
  ...
  if (status === "finalizado") {
    ...
    const _confirMsg = _totalActual === 0 ? "⚠️ Este pedido tiene total $0.00. ¿Deseas finalizarlo de todas formas sin precio registrado?" : "¿Marcar como finalizado? El pedido pasará al historial.";
...
async function confirmarAbonoPedido() {
  ...
  try {
    ...
    if (!monto || monto <= 0) {
      manekiToastExport("Ingresa un monto válido.", "warn");
      _abonoEnProceso = false;
      if (btn) {
        btn.disabled = false;
        btn.style.opacity = "";
...
function renderHistorialPedidos() {
  ...
  (function _calcTiempoPromedio() {
    ...
    el.textContent = conteo > 0 ? `⏱ Tiempo promedio de entrega: ${Math.round(totalDias / conteo)} días (basado en ${conteo} pedidos finalizados)` : "⏱ Sin suficientes datos de tiempo de entrega aún";
  })();
  ...
  lista.innerHTML = pageItems.length === 0 ? '<p class="text-center text-gray-400 py-6 text-sm">Sin pedidos en el historial</p>' : pageItems.map((p) => {
    ...
    const _botonesExtra = _esCancelado ? `<button onclick="reactivarPedido('${p.id}')" class="text-xs text-blue-500 hover:text-blue-700" title="Reactivar pedido cancelado">↩ Reactivar</button>` : `<button onclick="imprimirTicketPedido('${p.id}')" class="text-xs text-gray-400 hover:text-gray-600" title="Imprimir comprobante">🖨️</button>
...
async function cargarMasPedidosFinalizados() {
  ...
  try {
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = "Cargar más pedidos ↓";
    }
  }
}
...
function renderTopClientes() {
  ...
  el.innerHTML = top.length === 0 ? '<p class="text-xs text-gray-400 text-center py-2">Sin datos aún</p>' : top.map((c, i) => `<div class="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
...
(function patchPedidoFormForFinalizados() {
  ...
  document.getElementById("pedidoForm").addEventListener("submit", function(e) {
    ...
    if (idx === -1) {
      manekiToastExport("⚠️ No se encontró el pedido.", "warn");
      return;
    }
